...
const ElasticmapsESClient = {
  ...esClient,
  search: esClient.searchWithContext
};
...
InaturalistMapserver.prepareQuery = async req => {
  req.elastic_client = ElasticmapsESClient;
  req.elastic_query = { };
  ...
  switch ( req.params.style ) {
    ...
    case "points":
      ...
      if ( req.params.format === "torque.json" ) {
        ...
        if ( RedisCacheClient && RedisCacheClient.redisClient && RedisCacheClient.redisClient.get ) {
          req.redisCacheClient = RedisCacheClient.redisClient;
        }
...
InaturalistMapserver.beforeSendResult = async ( req, res ) => {
