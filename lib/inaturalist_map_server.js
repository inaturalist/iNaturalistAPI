...
InaturalistMapserver.prepareQuery = ( req, callback ) => {
  ...
  ObservationsController.reqToElasticQuery( req, ( err, query ) => {
    ...
    if ( req.params.format === "png" && req.params.style !== "points" ) {
      if ( req.query.skip_top_hits ) {
        req.skipTopHits = true;
      } else if ( req.query.style === "geogrid" ) {
        req.geogrid = true;
        req.params.dataType = "geojson";
...
InaturalistMapserver.prepareStyle = ( req, callback ) => {
  ...
  switch ( req.params.style ) {
    ...
    case "summary":
      ...
      break;
    case "colored_heatmap":
      tileOptions = Object.assign( { }, req.query );
      if ( req.inat.taxon && !req.query.color ) {
        tileOptions.color = Taxon.iconicTaxonColor( req.inat.taxon.iconic_taxon_id );
      }
      if ( req.geogrid ) {
        tileOptions.totalHits = req.totalHits;
        req.style = MapStyles.geotile( tileOptions );
      } else if ( req.geotile ) {
        tileOptions.totalHits = req.totalHits;
        req.style = MapStyles.geotile( tileOptions );
      } else {
