...
const DEFAULT_OBSERVATION_MAP_FIELDS = ["id", "uuid", "location", "taxon.iconic_taxon_id",
...
InaturalistMapserver.prepareQuery = async req => {
  ...
  if ( req.params.dataType === "postgis" || req.params.dataType === "geomodel" ) {
    req.largeTiles = ( req.query.tile_size === "512" );
  } else {
    req.query.order_by = "id";
    req.query.source = { includes: DEFAULT_OBSERVATION_MAP_FIELDS };
...
InaturalistMapserver.defaultRoute = async ( req, res ) => {
  ...
  try {
    ...
    if ( req.params.dataType === "geomodel" ) {
      const colorScale = customColorScale( "#97CAFF", "#1574D8", 0, 1 );
      req.csvData = await MapGenerator.pythonTilesGeojson( req, "h3_04", colorScale );
    } else if ( req.params.dataType === "geomodel_taxon_range" ) {
      const colorScale = chroma.scale( [chroma( "#FF5EB0" ).alpha( 0.4 )] );
      req.csvData = await MapGenerator.pythonTilesGeojson( req, "h3_04_taxon_range", colorScale );
