...
InaturalistMapserver.beforePrepareQuery = function( req, callback ) {
  req.sourceModifier = InaturalistMapserver.observationPartial;
...
InaturalistMapserver.beforeSendResult = function( req, res, callback ) {
  switch( req.params.style ) {
    case "places":
      res.setHeader( "Cache-Control", "public, max-age=2592000" ); // month
      break;
    case "summary":
    case "taxon_places":
      res.setHeader( "Cache-Control", "public, max-age=86400" ); // day
      break;
    default:
      res.setHeader( "Cache-Control", "public, max-age=3600" ); // hour
  };
...
var obsTemplate;
fs.readFile( "views/observation.jade", "utf8", function ( err, data ) {
  if( err ) { throw err; }
  obsTemplate = jade.compile( data );
});

InaturalistMapserver.observationPartial = function( obs ) {
  return obsTemplate( { obs: obs, util: util, moment: moment } );
};
