...
ElasticRequest.waitForReservedCount = async function (
  ...
  // add a very small up to 10ms random delay to help prevent a single map load
  // from tiggering many simultaneous tile requests which all trigger count
  // queries at exactly the same time
  await timersPromises.setTimeout( _.random( 0, 10, true ) );
