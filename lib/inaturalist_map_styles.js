...
InaturalistMapStyles.geotilegrid = options => {
  ...
  const count = _.max( [_.min( [100000, hits] ) / ( Math.pow( 2, options.zoom ) ), 3] );
  const key = JSON.stringify( { color: options.color, count } );
  if ( geotilegridStyles[key] ) {
    return geotilegridStyles[key];
  }
  const style = `polygon-fill: ${options.color || "#6e6e6e"}; `
    + "polygon-opacity: 0.2; "
    + "line-width: 1; "
    + "line-opacity: 0.5; "
    + "line-color: white; "
    + "line-comp-op: overlay; "
    + "[cellCount > 0] { polygon-opacity: 0.2; } "
    + `[cellCount > ${_.max( [1, count * Math.log10( 1.005 )] )}] { polygon-opacity: 0.3 } `
    + `[cellCount > ${_.max( [1, count * Math.log10( 1.01 )] )}] { polygon-opacity: 0.4 } `
    + `[cellCount > ${_.max( [1, count * Math.log10( 1.02 )] )}] { polygon-opacity: 0.45 } `
    + `[cellCount > ${_.max( [2, count * Math.log10( 1.05 )] )}] { polygon-opacity: 0.5 } `
    + `[cellCount > ${_.max( [2, count * Math.log10( 1.1 )] )}] { polygon-opacity: 0.55 } `
    + `[cellCount > ${_.max( [3, count * Math.log10( 1.2 )] )}] { polygon-opacity: 0.6 } `
    + `[cellCount > ${_.max( [3, count * Math.log10( 1.3 )] )}] { polygon-opacity: 0.65 } `
    + `[cellCount > ${_.max( [4, count * Math.log10( 1.4 )] )}] { polygon-opacity: 0.7 } `
    + `[cellCount > ${_.max( [4, count * Math.log10( 1.5 )] )}] { polygon-opacity: 0.75 } `
    + `[cellCount > ${_.max( [4, count * Math.log10( 1.8 )] )}] { polygon-opacity: 0.8 } `
    + `[cellCount > ${_.max( [5, count * Math.log10( 2 )] )}] { polygon-opacity: 0.9 } `
    + `[cellCount > ${_.max( [5, count * Math.log10( 4 )] )}] { polygon-opacity: 1 } `;
