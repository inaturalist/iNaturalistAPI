...
InaturalistMapStyles.geotile = options => {
...
InaturalistMapStyles.coloredHeatmap = options => {
  ...
  if ( options.scaled ) {
    ...
    style += `[cellCount > 0] { marker-fill-opacity: ${tileOptions.opacity}; } `
      ...
      + `[cellCount > ${tier7}] { marker-width: ${tileOptions.width * 0.8}; } `
      + `[cellCount > ${tier8}] { marker-width: ${tileOptions.width * 0.85}; } `
      + `[cellCount > ${tier9}] { marker-width: ${tileOptions.width * 0.9}; } `;
  }
