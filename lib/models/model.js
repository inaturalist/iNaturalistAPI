...
var Model = class Model {
      ...
      _.map( options.idFields, ( attrName, idField ) => {
        ...
        if( assocObj && assocObj.id &&
          ...
          association = new model(
            _.assignIn( { }, assocObj, resultsHash[ assocObj.id ] ), options );
        }
  ...
  static injectESHasMany( objects, model, resultsHash, options ) {
    options = options || { };
    const attrName = options.attrName || model.tableName;
    if ( !options.idFields ) {
      var modelID = options.foreignKey || model.modelName + "_id";
      options.idFields = { };
      options.idFields[ modelID ] = attrName;
    ...
    _.each( objects, function( o ) {
      ...
      if( !o ) { return; }
      _.map( options.idFields, ( attrName, idField ) => {
        _.each( o[ idField ], assocID => {
          if ( resultsHash[ assocID ] ) {
            const instance = new model( resultsHash[ assocID ], options );
            associations.push( instance );
          }
        });
      });
      if ( associations.length > 0 ) {
        o[attrName] = associations;
      }
    });
  }
  ...
  static preloadTaxonPhotoDimensions( taxa, callback ) {
    var ids = _.compact( _.uniq( _.flattenDeep( _.map( taxa, t => {
  ...
  static photoDimensionsByID( ids, callback ) {
    ...
    pgClient.connection.query( query.toString( ), ( err, result ) => {
      ...
      var photoSizeByID = _.fromPairs( _.map( result.rows, r => {
