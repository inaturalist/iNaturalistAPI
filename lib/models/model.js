...
const Model = class Model {
  ...
  static async preloadTaxonPhotoDimensions( taxa ) {
    const taxonPhotos = _.compact( _.flatten( _.map( taxa, "taxon_photos" ) ) );
    const photos = _.compact( _.flatten( _.map( taxonPhotos, "photo" ) ) );
    await Model.photoDimensionsByID( photos );
  }

  static async photoDimensionsByID( photos ) {
    const ids = _.compact( _.uniq( _.map( photos, "id" ) ) );
    ...
    const photoSizeByID = _.fromPairs( _.map( rows, r => {
