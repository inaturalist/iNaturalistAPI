...
const RedisConnection = class RedisConnection {
  ...
  async connect( ) {
    if ( ( this.redisClient && this.connected ) || this.connectFailed ) {
  ...
  async awaitConnection( numberOfAttempts = 1 ) {
    RedisConnection.debug( "Awaiting Redis connection..." );
  ...
  async del( key ) {
    ...
    const queryTime = hrtime.bigint( ) - startTime;
    const runtime = _.round( Number( queryTime / BigInt( 1000 ) ) / 1000, 1 );
    RedisConnection.debug( `  ${chalk.magenta.bold( `[redis] (${runtime}ms)` )} ${chalk.green.bold( "Deleting from cache:" )} ${chalk.yellow.bold( key )}` );
  }
