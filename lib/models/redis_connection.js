...
const RedisConnection = class RedisConnection {
  ...
  async connect( ) {
    ...
    if ( config.redis && config.redis.port && config.redis.host ) {
      RedisConnection.debug( "Connecing to redis..." );
  ...
  async getOrSetJSON( key, expiration, asyncMethod ) {
    ...
    if ( cachedJSON ) {
      if ( process.env.NODE_ENV !== "test" ) {
        ...
        console.log( `  ${chalk.magenta.bold( `[redis] (${runtime}ms)` )} ${chalk.green.bold( "Returning cached:" )} ${chalk.yellow.bold( key )} ${chalk.green.bold( `[ttl: ${remainingTTL}]` )}` );
      }
      return cachedJSON;
    }
    ...
    const queryTime = hrtime.bigint( ) - startTime;
    const runtime = _.round( Number( queryTime / 1000n ) / 1000, 1 );
    console.log( `  ${chalk.magenta.bold( `[redis] (${runtime}ms)` )} ${chalk.green.bold( "Adding to cache:" )} ${chalk.yellow.bold( key )} ${chalk.green.bold( `[ttl: ${expiration}]` )}` );
