...
const ESModel = class ESModel {
  ...
  static async ancestriesSpeciesByKingdom( req, field, searchMethod, kingdomId ) {
    const countQueryWithKingdom = ( kingdomId.length === 1 )
      ? _.assignIn( { }, req.query, { taxon_id: kingdomId } )
      : _.assignIn( { }, req.query, { taxon_ids: kingdomId } );
  ...
  static async ancestriesSpeciesCountsFetch( req, field, searchMethod ) {
    ...
    fetchPromises.push( ESModel.ancestriesSpeciesByKingdom(
      req,
      field,
      searchMethod,
      [47170, 48222, 67333, 47686, 131236, 151817]
    ) );
