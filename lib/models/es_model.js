...
const ESModel = class ESModel {
  ...
  static async mget( ids, index, source = { } ) {
    let mgetParams = {
      preference: global.config.elasticsearch.preference,
      index: `${process.env.NODE_ENV || global.config.environment}_${index}`,
      body: { ids }
    ...
    if ( !_.isEmpty( source ) ) {
      mgetParams = Object.assign( mgetParams, source );
    }
    return esClient.connection.mget( mgetParams );
  }

  static async mgetResults( ids, index, source = { } ) {
    const getResponse = await ESModel.mget( ids, index, source );
    return {
      hits: {
        total: {
