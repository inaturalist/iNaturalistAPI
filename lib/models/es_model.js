...
const ESModel = class ESModel {
  ...
  static async userObservedTaxonIDsCached( userID ) {
    return new Promise( ( resolve, reject ) => {
      ...
      const fileCacheKey = `taxa-observed-by-${userID}`;
      const cb = ( err, data ) => {
        if ( err ) { return void reject( err ); }
        resolve( data );
      };
      CachemanCache.cachedOrFetchFile( fileCacheKey, ( ) => {
        ESModel.userObservedTaxonIDs( userID ).then( taxonIDs => {
          CachemanCache.cachedCallbackFile( fileCacheKey, 60 * 60, cb )( null, taxonIDs );
        } );
      }, cb );
    } );
  }
