...
var ESModel = class ESModel {
  ...
  static fetchInstancesByIDs( ids, model, options, callback ) {
  ...
  static fetchBelongsTo( objects, model, options, callback ) {
    ...
    ESModel.fetchInstancesByIDs( ids, model, options, ( err, resultsHash ) => {
      if( err ) { return callback( err ); }
  ...
  static fetchHasMany( objects, model, options, callback ) {
    options = options || { };
    const foreignKey = options.foreignKey || model.modelName + "_id";
    var ids = _.uniq( _.compact( _.flatten( _.map( objects, foreignKey ) ) ) );
    ESModel.fetchInstancesByIDs( ids, model, options, ( err, resultsHash ) => {
      if( err ) { return callback( err ); }
      Model.injectESHasMany( objects, model, resultsHash, options );
      callback( );
    });
  }
  ...
  static fetchInstancesByIDsObject( idsObject, model, options, callback ) {
    ...
    ESModel.fetchInstancesByIDs( ids, model, options, callback );
  }
