...
const ESModel = class ESModel {
  ...
  static ancestriesSpeciesCounts( req, field, searchMethod, callback ) {
    ...
    if ( CachemanCache && searchMethod.name === "observationElasticResults" ) {
      const queryDup = _.pickBy( _.cloneDeep( req.query ), a => ( a !== "any" && !_.isEmpty( a ) ) );
      const reqInatDup = _.pickBy( _.cloneDeep( req.inat ), a => ( a !== "any" && !_.isEmpty( a ) ) );
  ...
  static userAggregationResponse( req, countQuery, searchMethod, options, callback ) {
    ...
    const {
      page, perPage, offset
    } = InaturalistAPI.paginationData( req, options.paginationOptions );
    searchMethod( countReq, ( err, data ) => {
      if ( err ) { return void callback( err ); }
      ...
      const results = _.sortBy( buckets, b => -1 * b.count )
        .slice( offset, offset + perPage );
      ESModel.fetchBelongsTo( results, User, { }, errr => {
