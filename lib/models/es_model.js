...
const ESModel = class ESModel {
  ...
  static async ancestriesSpeciesByKingdom( req, field, searchMethod, kingdomId ) {
    const countQueryWithKingdom = _.assignIn( { }, req.query, { taxon_id: kingdomId } );
    const countQuery = _.assignIn( { }, countQueryWithKingdom, {
      per_page: 0,
    ...
    const ancestriesBuckets = data.aggregations.ancestries.buckets;

    const kingdomPromiseResult = { 
      resultTaxonIDs: resultTaxonIDs,
      ancestriesBuckets: ancestriesBuckets
    };
    
    return kingdomPromiseResult;
  }

  static async ancestriesSpeciesCountsFetch( req, field, searchMethod ) {
    let kingdomPromises = [];
    let resultTaxonIDs = [];
    let ancestriesBuckets = [];
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [47126] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [47170] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [1] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [48222] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [67333] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [47686] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [131236] ) );
    kingdomPromises.push( ESModel.ancestriesSpeciesByKingdom( req, field, searchMethod, [151817] ) );

    const kingdomData = await Promise.all(kingdomPromises);

    kingdomData.forEach( kData => {
      resultTaxonIDs = resultTaxonIDs.concat( kData.resultTaxonIDs );
      ancestriesBuckets = ancestriesBuckets.concat( kData.ancestriesBuckets );
    } );
    ...
    _.each( ancestriesBuckets, bucket => {
