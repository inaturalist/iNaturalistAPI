...
const ESModel = class ESModel {
  ...
  static fetchBelongsToSub( objects, ids, model, options ) {
  ...
  static fetchHasManySub( objects, ids, InjectModel, foreignKey, options ) {
    return new Promise( ( resolve, reject ) => {
      if ( _.isEmpty( ids ) ) { return void resolve( ); }
  ...
  static ancestriesSpeciesCounts( req, field, searchMethod, callback ) {
    ...
    if ( searchMethod.name === "observationElasticResults" ) {
      fileCacheKey = util.observationSearchRequestCacheKey(
        req, "ancestriesSpeciesCounts", { ignorePagination: true }
      );
    }
    CachemanCache.cachedOrFetchFile( fileCacheKey, ( ) => {
      ESModel.ancestriesSpeciesCountsFetch( req, field, searchMethod,
        CachemanCache.cachedCallbackFile( fileCacheKey, 60 * 60 * 24, callback ) );
    }, callback );
  }

  static ancestriesSpeciesCountsFetch( req, field, searchMethod, callback ) {
