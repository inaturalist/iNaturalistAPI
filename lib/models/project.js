...
const Project = class Project extends Model {
  ...
  async members( options ) {
    ...
    await ESModel.fetchBelongsToAsync( projectUsers, User, {
      source: {
  ...
  static async preloadForRules( projects, options ) {
    ...
    if ( options.skipSubprojects !== false ) {
      ...
      if ( !_.isEmpty( umbrellaProjects ) ) {
        ...
        await ESModel.fetchBelongsToAsync( projectRules, Project, { foreignKey: "operand_id" } );
    ...
    await Promise.all( [
      ESModel.fetchBelongsToAsync( projects, Place, { source: { excludes: ["geometry_geojson"] } } ),
      ESModel.fetchBelongsToAsync( taxonRules, Taxon, taxonOpts ),
      ESModel.fetchBelongsToAsync( userRules, User, { foreignKey: "operand_id" } ),
      ESModel.fetchBelongsToAsync( placeRules, Place, placeOpts ),
      ESModel.fetchBelongsToAsync( projectRules, Project, { foreignKey: "operand_id" } ),
      ESModel.fetchBelongsToAsync( controlledTermRules, ControlledTerm, { foreignKey: "value" } )
    ] );
  }
};
