...
var Project = class Project extends Model {
  ...
  static findAllByIDElastic( id, callback ) {
    ...
    ESModel.fetchInstancesByIDs( ids, Project, options, ( err, resultsHash ) => {
      if ( err ) { return callback( err ); }
  ...
  static preloadForRules( projects, options, callback ) {
    ...
    const placeOpts = {
      source: { excludes: [ "geometry_geojson", "bounding_box_geojson", "point_geojson" ] },
      foreignKey: "operand_id"
    };
    const taxonRules =[];
    const userRules =[];
    ...
    const withUsers = projects.concat( _.flatten( _.map( projects, "admins" ) ) );
    ESModel.fetchBelongsTo( projects, Place, { source: { excludes: [ "geometry_geojson", "bounding_box_geojson" ] } }, err => {
      if( err ) { return callback( err ); }
