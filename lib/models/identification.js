...
const Identification = class Identification extends Model {
  ...
  static async preloadInto( arr, localeOpts ) {
    ...
    await DBModel.fetchBelongsToAsync( arr, Identification );
    const identification = _.map( arr, "identification" );
    await ESModel.fetchBelongsToAsync( identification, Taxon, taxonOpts );
    await ESModel.fetchBelongsToAsync( identification, User );
  }
};
