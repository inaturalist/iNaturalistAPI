...
var Taxon = class Taxon extends Model {
  ...
  preferredCommonName( options ) {
    ...
    var nameInPlaceInLocale;
    var nameInAncestorPlaceInLocale;
    var namesInLocale = [];
    var namesInPlace = [];
    ...
    if ( namesInPlace.length > 0 ) {
      ...
      let taxonNameInPlaceInLocale = _.find( sortedNamesInPlace, n => n.locale === options.locale );
    ...
    if ( namesInAncestorPlace.length > 0 ) {
      ...
      if ( sortedNamesInAncestorPlace.length > 0 ) {
        nameInAncestorPlace = sortedNamesInAncestorPlace[0].name;
      }
      let taxonNameInAncestorPlaceInLocale = _.find( sortedNamesInAncestorPlace, n => n.locale === options.locale );
    ...
    nameInLocale = nameInPlaceInLocale || nameInPlace || nameInAncestorPlaceInLocale || nameInLocale;

    if (
      options.defaultToEnglish === true &&
      !nameInLocale &&
      options.locale != "en"
    ) {
