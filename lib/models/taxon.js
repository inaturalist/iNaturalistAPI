...
const Taxon = class Taxon extends Model {
  ...
  preferredCommonName( opts = { } ) {
    ...
    _.each( this.names, n => {
      if ( ( !n.locale && !options.locale )
          || ( n.locale && n.locale.toLowerCase( ) === options.locale ) ) {
  ...
  static preloadBasicInfoParallel( taxa, concurrency = 3 ) {
    return new Promise( ( resolve, reject ) => {
      const chunks = _.chunk( _.values( taxa ), 1000 );
      const promiseProducer = ( ) => {
        const chunk = chunks.shift( );
        ...
        return Taxon.preloadBasicInfo( chunk );
      };
      const pool = new PromisePool( promiseProducer, concurrency );
      pool.start( ).then( ( ) => {
        resolve( );
  ...
  static preloadBasicInfo( taxa ) {
    return new Promise( ( resolve, reject ) => {
      if ( taxa.length === 0 ) { return void resolve( ); }
      const taxonIDs = _.map( taxa, "id" );
      if ( _.isEmpty( taxonIDs ) ) { return void resolve( ); }
      const query = squel.select( )
        .field( "id, name, rank, rank_level, ancestry" )
        .from( "taxa" )
