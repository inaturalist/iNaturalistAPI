...
const Taxon = class Taxon extends Model {
  ...
  preferredCommonNameByPreference( taxonNamePreference, options = { } ) {
    const namesInLexicon = [];
    const namesInPlace = [];
    const namesInAncestorPlace = [];
    _.each( this.names, n => {
      if ( taxonNamePreference.lexicon ) {
        if ( n.lexicon === taxonNamePreference.lexicon ) {
      ...
      if ( taxonNamePreference.place_id && n.place_taxon_names ) {
        const ptn = _.find( n.place_taxon_names,
          pn => pn.place_id === taxonNamePreference.place_id );
        if ( ptn ) {
          namesInPlace.push( { ...n, sortKey: ptn.position } );
        } else {
          const ancestorPtn = _.find( n.place_taxon_names, pn => (
            _.includes( taxonNamePreference.ancestor_place_ids, pn.place_id )
          ) );
          if ( ancestorPtn ) {
    ...
    if ( namesInPlace.length > 0 ) {
      ...
      if ( taxonNamePreference.lexicon ) {
        nameInPlaceInLexicon = _.find(
          sortedNamesInPlace, n => n.lexicon === taxonNamePreference.lexicon
        );
      } else if ( options.locale ) {
        nameInPlaceInLexicon = _.find(
    ...
    if ( namesInAncestorPlace.length > 0 ) {
      ...
      if ( taxonNamePreference.lexicon ) {
        nameInAncestorPlaceInLexicon = _.find(
          sortedNamesInAncestorPlace, n => n.lexicon === taxonNamePreference.lexicon
        );
      } else if ( options.locale ) {
        nameInAncestorPlaceInLexicon = _.find(
    ...
    nameInLexicon = nameInPlaceInLexicon
      || ( !taxonNamePreference.lexicon && nameInPlace )
    ...
    if ( !taxonNamePreference.lexicon && options.locale && options.locale.match( /-/ ) ) {
      const nameInBaseLocale = this.preferredCommonNameByPreference(
        taxonNamePreference, { locale: options.locale.split( "-" )[0] }
      );
      if ( nameInBaseLocale ) {
  ...
  preferredCommonNames( options = { } ) {
    if ( !options.userSession
      ...
      || _.isEmpty( options.userSession.taxonNamePreferences ) ) {
      return null;
    }
    const preferredCommonNames = [];
    _.each( _.sortBy( options.userSession.taxonNamePreferences, "position" ), taxonNamePreference => {
      const preferredName = this.preferredCommonNameByPreference( taxonNamePreference, options );
      if ( preferredName ) {
  ...
  preferredCommonName( opts = { } ) {
    ...
    const stubTaxonNamePreference = {
      lexicon: null
    };
    if ( options.preferredPlace ) {
      stubTaxonNamePreference.place_id = options.preferredPlace.id;
      stubTaxonNamePreference.ancestor_place_ids = options.preferredPlace.ancestor_place_ids;
    }
    const preferredTaxonName = this.preferredCommonNameByPreference(
      stubTaxonNamePreference, { locale: options.locale }
    );
  ...
  prepareForResponse( localeOptions = { }, opts = { } ) {
    ...
    this.preferred_common_names = this.preferredCommonNames( localeOpts );
