...
var Taxon = class Taxon extends Model {
  ...
  preferredCommonName( options ) {
    ...
    _.each( this.names, function( n ) {
      if ( n.locale !== options.locale ) { return; }
    ...
    if ( namesInPlace.length > 0 ) {
      nameInPlace = _.sortBy( namesInPlace, n => n.sortKey )[0].name;
    }
    if ( namesInAncestorPlace.length > 0 ) {
      nameInAncestorPlace = _.sortBy( namesInAncestorPlace, n => n.sortKey )[0].name;
    }
  ...
  conservationStatus( place ) {
    ...
    _.each( this.statuses, function( s ) {
      ...
      if( place ) {
        if( _.includes( place.ancestor_place_ids, s.place_id ) && s.iucn > ancestorStatus ) {
          ancestorStatus = s;
        }
  ...
  static assignConservationStatuses( taxa, options, callback ) {
    ...
    var query = squel.select( ).
      field( "cs.taxon_id, cs.status, cs.authority, cs.iucn, " +
        ...
        "p.display_name place_display_name, p.ancestry place_ancestry, " + 
        "p.admin_level place_admin_level" ).
