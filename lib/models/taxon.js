...
const Taxon = class Taxon extends Model {
  ...
  static assignTaxonCurators( taxa, options, callback ) {
    if ( !options.details ) { return void callback( null, taxa ); }
    const ancestorIds = _.uniq( _.compact( _.flattenDeep( _.map(
      taxa, t => [t.id, t.ancestor_ids]
    ) ) ) );
    if ( _.isEmpty( ancestorIds ) ) { return void callback( null, taxa ); }
    const query = squel.select( )
      .field( "tc.user_id" )
      .field( "tc.taxon_id" )
    ...
    pgClient.connection.query( query.toString( ), ( err, result ) => {
      ...
      _.each( _.compact( taxa ), taxon => {
        const curatorsOfTaxon = _.filter( curatorsWithRanks, curator => {
          const curatorOfAncestor = curator.taxon_id === taxon.id
            || taxon.ancestor_ids.indexOf( curator.taxon_id ) >= 0;
          const curatorOfRank = !curator.completeRankLevel
            || curator.completeRankLevel <= taxon.rank_level;
          return curatorOfAncestor && curatorOfRank;
        } );
        if ( curatorsOfTaxon.length > 0 ) {
          taxon.taxon_curators = curatorsOfTaxon;
          _.each( curatorsOfTaxon, c => {
            usersByID[c.user_id] = usersByID[c.user_id] || { id: c.user_id };
          } );
        }
      } );
      User.findByIDInES( _.keys( usersByID ), ( errr, users ) => {
