...
const User = class User extends Model {
  ...
  static async privateDbAttributesFromLogin( login ) {
    ...
    const namePreferencesQuery = squel.select( )
      .field( "id, position, user_id, place_id, lexicon" )
      .from( "taxon_name_preferences" )
      .order( "position" )
      .where( "user_id = ?", userObject.id );
    const { rows: namePreferencesRows } = await pgClient.query( namePreferencesQuery.toString( ) );
    userObject.taxon_name_preferences = namePreferencesRows;
    return userObject;
  }
  ...
  static async localeDefaults( userID ) {
    ...
    defaults.taxonNamePreferences = await User.taxonNamePreferences( userID );
    return defaults;
  }

  static async taxonNamePreferences( userID ) {
    const query = squel.select( )
      .field( "taxon_name_preferences.position" )
      .field( "taxon_name_preferences.place_id" )
      .field( "taxon_name_preferences.lexicon" )
      .field( "places.name", "place_name" )
      .field( "places.ancestry", "place_ancestry" )
      .from( "taxon_name_preferences" )
      .left_join( "places", null, "taxon_name_preferences.place_id = places.id" )
      .where( "taxon_name_preferences.user_id = ?", userID );
    const { rows } = await pgClient.query( query.toString( ) );
    _.each( rows, row => {
