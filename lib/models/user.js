...
const User = class User extends Model {
  ...
  static async privateDbAttributesFromLogin( login ) {
    ...
    const { rows: prefRows } = await pgClient.query( prefsQuery.toString( ) );
    _.each( PREFS, pref => {
    ...
    const { rows: namePrioritiesRows } = await pgClient.query( namePriorityQuery.toString( ) );
  ...
  static async blocks( userID ) {
    ...
    let query = squel.select( )
      ...
      .where( "user_id = ?", userID );
    let { rows } = await pgClient.query( query.toString( ) );
    _.map( rows, r => {
    ...
    ( { rows } = await pgClient.query( query.toString( ) ) );
    _.map( rows, r => {
