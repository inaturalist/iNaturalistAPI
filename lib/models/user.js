...
const PREFS = [
  ...
  { name: "infraspecies_identification_notifications", default: true },
  ...
  { name: "non_disagreeing_identification_notifications", default: true },
...
const PREFERS_PREFS = [
  ...
  "infraspecies_identification_notifications",
  "mention_email_notification",
  "message_email_notification",
  "monthly_supporter_badge",
  "no_email",
  "no_tracking",
  "non_disagreeing_identification_notifications",
...
const User = class User extends Model {
  ...
  static async privateDbAttributesFromLogin( login ) {
    ...
    const query = squel.select( )
      ...
      .field( "fundraiseup_plan_status" )
      .field( "fundraiseup_plan_frequency" )
      .field( "users.id" )
      .field( "users.search_place_id" )
      .field( "users.confirmed_at" )
      .field( "users.confirmation_sent_at" )
      ...
      .field( "pf.project_id", "pf_project_id" )
      .field( "pf.position", "pf_position" )
      .field( "es.suppression_type", "es_suppression_type" )
      .from( "users" )
      ...
      .left_join( "project_faves", "pf", "users.id = pf.user_id" )
      .left_join( "email_suppressions", "es", "users.id = es.user_id" )
      .where( "login = ?", login );
    const { rows } = await pgClient.replica.query( query.toString( ) );
    ...
    const { rows: prefRows } = await pgClient.replica.query( prefsQuery.toString( ) );
    _.each( PREFS, pref => {
    ...
    userObject.faved_project_ids = _.compact(
      _.uniq(
        _.map(
          _.sortBy( rows, "pf_position" ),
          r => r.pf_project_id
        )
      )
    );
    userObject.email_suppression_types = _.compact(
      _.uniq(
        _.map(
          _.sortBy( rows, "es_suppression_type" ),
          r => r.es_suppression_type
        )
      )
    );
    delete userObject.pf_project_id;
    delete userObject.pf_position;
    userObject.monthly_supporter = ( (
      userObject.donorbox_plan_type === "monthly"
      && userObject.donorbox_plan_status === "active"
    ) || (
      userObject.fundraiseup_plan_frequency === "monthly"
      && userObject.fundraiseup_plan_status === "active"
    ) );
    delete userObject.donorbox_plan_status;
    delete userObject.donorbox_plan_type;
    delete userObject.fundraiseup_plan_status;
    delete userObject.fundraiseup_plan_frequency;

    const namePriorityQuery = squel.select( )
      .field( "id, position, user_id, place_id, lexicon" )
      .from( "taxon_name_priorities" )
      .order( "position" )
      .where( "user_id = ?", userObject.id );
    const { rows: namePrioritiesRows } = await pgClient.replica.query(
      namePriorityQuery.toString( )
    );
    userObject.taxon_name_priorities = namePrioritiesRows;
    return userObject;
  }
  ...
  static async findByLoginOrID( login ) {
    if ( _.isEmpty( login ) && !_.isNumber( login ) ) { return null; }
  ...
  static iconUrl( user ) {
    ...
    const prefix = config.userImagePrefix
  ...
  static async blocks( userID ) {
    ...
    let query = squel.select( )
      .field( "blocked_user_id, blocked_users.login AS blocked_user_login" )
      .from( "user_blocks" )
      .join( "users", "blocked_users", "user_blocks.blocked_user_id = blocked_users.id" )
      .where( "user_id = ?", userID );
    let { rows } = await pgClient.replica.query( query.toString( ) );
    _.map( rows, r => {
    ...
    query = squel.select( )
      .field( "user_id, blocked_user_id, users.login AS user_login" )
      .from( "user_blocks" )
      .join( "users", "users", "user_blocks.user_id = users.id" )
      .where( "blocked_user_id = ?", userID );
    ( { rows } = await pgClient.replica.query( query.toString( ) ) );
    _.map( rows, r => {
  ...
  static async localeDefaults( userID ) {
    const query = squel.select( )
      ...
      .left_join( "(roles_users ru_admins join roles ru_admins_roles ON (ru_admins.role_id = ru_admins_roles.id AND ru_admins_roles.name = 'admin'))",
        null,
        "users.id = ru_admins.user_id" )
      .left_join( "(roles_users ru_curators join roles ru_curators_roles ON (ru_curators.role_id = ru_curators_roles.id AND ru_curators_roles.name = 'curator'))",
        null,
    ...
    if ( row.confirmed_at ) { defaults.emailConfirmed = true; }
    if ( row.place_id ) {
  ...
  static async taxonNamePriorities( userID ) {
    const query = squel.select( )
      .field( "taxon_name_priorities.position" )
      .field( "taxon_name_priorities.place_id" )
      .field( "taxon_name_priorities.lexicon" )
      .field( "places.name", "place_name" )
      .field( "places.ancestry", "place_ancestry" )
      .from( "taxon_name_priorities" )
      .left_join( "places", null, "taxon_name_priorities.place_id = places.id" )
      .where( "taxon_name_priorities.user_id = ?", userID );
    const { rows } = await pgClient.replica.query( query.toString( ) );
    _.each( rows, row => {
      row.ancestor_place_ids = row.place_ancestry
        ? row.place_ancestry.split( "/" ).map( Number ) : [];
    } );
    return rows;
  }
