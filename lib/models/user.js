...
var User = class User extends Model {
  ...
  static iconUrl( user ) {
    if( !user.local_icon_content_type ) { return; }
    var extension;
    if( user.local_icon_content_type && user.local_icon_content_type.match( /jpeg$/ ) ) { extension = "jpg"; }
    else if( user.local_icon_content_type && user.local_icon_content_type.match( /png$/ ) ) { extension = "png"; }
    else if( user.local_icon_content_type && user.local_icon_content_type.match( /gif$/ ) ) { extension = "gif"; }
    else if( user.local_icon_content_type && user.local_icon_content_type.match( /bmp$/ ) ) { extension = "bmp"; }
    else if( user.local_icon_content_type && user.local_icon_content_type.match( /tiff$/ ) ) { extension = "tiff"; }
    else return;
    // catch a few file_names that paperclip knows are JPEGs
    if( extension == "jpg" && user.local_icon_file_name &&
        user.local_icon_file_name.match( /(stringio\.txt|open-uri|\.jpeg|^data$)/ ) ) {
      extension = "jpeg";
    }
...
User.returnFields = [
  "id", "login", "name", "local_icon_content_type", "local_icon_file_name" ];
