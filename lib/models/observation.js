...
var Observation = class Observation extends Model {
  ...
  prepareObservationPhotos( ) {
    if( this.observation_photos ) {
      var photosByID = _.fromPairs( _.map( this.photos, p => [ p.id, p ] ) );
  ...
  // STOPGAP until we get observation photo IDs into the index or the apps transition to using UUIDs
  static preloadObservationPhotos( obs, callback ) {
    if ( obs.length === 0 ) { return callback( null ); }
    const obsIDs = _.uniq( _.compact( obs.map( o => o.id ) ) );
    if ( _.isEmpty( obsIDs ) ) { return callback( null ); }
    const query = squel.select( ).fields([ "id, observation_id, photo_id" ]).
    ...
    pgClient.connection.query( query.toString( ), ( err, result ) => {
      ...
      _.each( result.rows, r => {
        ...
        obsPhotos[r.observation_id][r.photo_id] = r.id;
      } );
      // loop through each obs in the ES response
      _.each( obs, o => {
        // the obs has photos from the DB
        if ( obsPhotos[o.id] ) {
          // loop through the obs photos from the ES response
          _.each( o.observation_photos, obsPhoto => {
            if ( obsPhoto.photo && obsPhoto.photo.id && obsPhotos[o.id][obsPhoto.photo.id] ) {
              // the obs photo from ES matched one from the DB,
              // so assign it the obs photo ID from the DB
              obsPhoto.id = obsPhotos[o.id][obsPhoto.photo.id];
            }
          } );
        }
      });
    ...
    ESModel.fetchBelongsTo( withTaxa, Taxon, taxonOpts, err => {
      ...
      TaxaController.assignAncestors( { }, taxa, { localeOpts, ancestors: true }, err => {
        ...
        ESModel.fetchBelongsTo( withUsers, User, { }, err => {
          if( err ) { return callback( err ); }
          Observation.preloadObservationPhotos( withUsers, callback );
        } );
      });
    });
  }
