...
var Observation = class Observation extends Model {
  ...
  static photoAttribution( photo, attributionName, licenseID ) {
    const license = LICENSE_INFO[licenseID] || LICENSE_INFO[0];
    if ( license.code === "pd" ) {
      return `${attributionName}, no known copyright restrictions (public domain)`;
    } else if ( license.code === "cc0" ) {
      return "no rights reserved";
    } else if ( license.code && license.code.match( /^cc-/ ) ) {
  ...
  static preloadObservationPhotos( obs, callback ) {
    ...
    pgClient.connection.query( query.toString( ), ( err, result ) => {
      ...
      _.each( result.rows, r => {
        ...
        photo.attribution = Observation.photoAttribution( photo, photoAttributionName, r.license );
