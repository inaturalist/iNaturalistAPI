...
var _ = require( "lodash" ),
    ...
    Taxon = require( "./taxon" ),
    turfCircle = require( "@turf/circle" ),
...
var Observation = class Observation extends Model {
  ...
  removeUnviewableGeo( options = { } ) {
    ...
    // Remove places that do not contain the visible area of uncertainty
    let acc = this.public_positional_accuracy;
    let geojson = this.geojson;
    if ( viewerCanSeePrivateCoordinates ) {
      acc = this.positional_accuracy;
      geojson = this.private_geojson || geojson;
    }
    if ( geojson && acc ) {
      const that = this;
      const o = Object.assign( {}, this );
      esClient.search( "places", { body: {
          query: { bool: { filter: { terms: { id: o.place_ids }} } },
          _source: ["id", "name", "bounding_box_geojson"],
          size: o.place_ids.length } },
        function( err, results ) {
          if( err ) {
            // If something goes wrong, play it safe and remove all place IDs
            delete that.place_ids;
            return;
          }
          // only include place IDs
          const newPlaceIDs = _.compact( _.map( results.hits.hits, hit => {
            const place = hit._source;
            const bbox = place.bounding_box_geojson;
            if ( !bbox ) {
              return null;
            }
            const circle = turfCircle( geojson, acc, { units: "meters" } );
            return turfBooleanContains( bbox, circle ) ? place.id : null;
          } ) );
          that.place_ids = newPlaceIDs;
      });
    }
    if ( !viewerCanSeePrivateCoordinates ) {
  ...
  static preloadAllAssociations( obs, localeOpts, callback ) {
    Observation.preloadAnnotationControlledTerms( obs, err => {
      ...
      Observation.preloadObservationFields( obs, err => {
        if( err ) { return callback( err ); }
        const projobs = _.flattenDeep( _.map( obs, o => {
