...
const Observation = class Observation extends Model {
  ...
  removeUnviewableGeo( options = { } ) {
    ...
    if ( options.userSession && this.user ) {
      ...
      if ( viewerIsObserver || viewerHasCuratorCoordinateAccess || viewerTrustedByObserver ) {
  ...
  static async preloadInto( arr, options ) {
    await ESModel.fetchBelongsToAsync( arr, Observation, options );
    ...
    await preloadCallback( observations, options );
  }
  ...
  static async preloadAnnotationControlledTerms( obs ) {
    return ESModel.fetchBelongsToAsync(
  ...
  static async preloadAllAssociations( obs, localeOpts ) {
    ...
    await Promise.all( [
      ESModel.fetchBelongsToAsync( withProjects, Project, { source: Project.returnFields } ),
      ...
      ESModel.fetchBelongsToAsync( obs, Taxon, taxonOpts )
    ] );
    _.each( obs, o => {
  ...
  static async preloadMinimal( obs, localeOpts ) {
    ...
    await ESModel.fetchBelongsToAsync( withTaxa, Taxon, taxonOpts );
    const taxa = _.compact( _.map( ids, "taxon" ) );
    await Promise.all( [
      TaxaController.assignAncestors( { }, taxa, { localeOpts, ancestors: true } ),
      ESModel.fetchBelongsToAsync( withUsers, User, { } ),
