...
var Observation = class Observation extends Model {
  ...
  static preloadAnnotationControlledTerms( obs, callback ) {
    ESModel.fetchBelongsTo(
      _.flatten( _.map( obs, "annotations" ) ),
      ControlledTerm,
      { idFields: {
  ...
  static preloadMinimal( obs, localeOpts, callback ) {
    ...
    const ids = _.flatten( _.map( obs, "identifications" ) );
    const comments = _.flatten( _.map( obs, "comments" ) );
    const ofvs = _.flatten( _.map( obs, "ofvs" ) );
    var withTaxa = _.filter(
      _.flatten( [ obs, ids, ofvs ] ), wt => ( wt && (
        ( wt.taxon && Number( wt.taxon.id ) ) ||
        ( wt.taxon_id && Number( wt.taxon_id ) ) ) )
    );
    var annotations = _.flatten( _.map( obs, "annotations" ) );
    var annotationVotes = _.flatten( _.map( annotations, "votes" ) );
