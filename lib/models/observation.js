...
const Observation = class Observation extends Model {
  ...
  removeUnviewableGeo( options = { } ) {
    ...
    if ( options.userSession && this.user ) {
      ...
      if (
        ...
        && this.non_traditional_projects
        && this.non_traditional_projects.length > 0
      ) {
        viewerCuratesCollectionProjectObserverTrusts = _.find( this.non_traditional_projects, p => {
          const projectUser = _.find( this.user.project_users, pu => pu.project.id === p.id );
          return (
            projectUser
            && projectUser.preferences
            && projectUser.preferences.curator_coordinate_access_for
            && (
              projectUser.preferences.curator_coordinate_access_for === "any"
              || (
                projectUser.preferences.curator_coordinate_access_for === "taxon"
                && ["obscured", "private"].indexOf( this.taxon_geoprivacy ) >= 0
              )
            )
          );
        } );
  ...
  static async preloadAllAssociations( obs, localeOpts ) {
    ...
    await Promise.all( [
      ESModel.fetchBelongsTo( withProjects, Project, { source: Project.returnFields } ),
