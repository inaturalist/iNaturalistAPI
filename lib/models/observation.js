...
const Observation = class Observation extends Model {
  ...
  removeUnviewableGeo( options = { } ) {
    ...
    if ( options.userSession && this.user ) {
      ...
      const viewerHasCoordinateAccessPrivilege = options.userSession.userPrivileges
        && ( !this.geoprivacy || this.geoprivacy === "open" )
        && options.userSession.userPrivileges.indexOf( "coordinate_access" ) >= 0
        && options.userSession.isAdmin;
      if (
        viewerIsObserver
        || viewerHasCuratorCoordinateAccess
        || viewerTrustedByObserver
        || viewerHasCoordinateAccessPrivilege
      ) {
