...
const Observation = class Observation extends Model {
  ...
  prepareObservationPhotos( ) {
    if ( this.observation_photos ) {
      const photosByID = _.fromPairs( _.map( this.photos, p => [p.id, p] ) );
      _.each( this.observation_photos, op => {
        op.photo = photosByID[op.photo_id];
        if ( op.photo ) {
          op.photo.url = util.fixHttps( op.photo.url );
        }
        delete op.photo_id;
      } );
    }
  ...
  static preloadInto( arr, options, callback ) {
    ESModel.fetchBelongsTo( arr, Observation, options, ( ) => {
      const observations = _.map( arr, "observation" );
  ...
  static preloadAllAssociations( obs, localeOpts, callback ) {
    Observation.preloadAnnotationControlledTerms( obs, err => {
      if ( err ) { return void callback( err ); }
      Observation.preloadObservationFields( obs, err2 => {
        ...
        ESModel.fetchBelongsTo( withProjects, Project, { source: Project.returnFields }, err3 => {
          ...
          Observation.preloadProjectMembership( obs, localeOpts, err4 => {
            if ( err4 ) { return void callback( err4 ); }
            Observation.preloadObservationUserPreferences( obs, err5 => {
              if ( err5 ) { return void callback( err5 ); }
              Observation.preloadObservationApplications( obs, err6 => {
                if ( err6 ) { return void callback( err6 ); }
                Observation.preloadMinimal( obs, localeOpts, err7 => {
  ...
  static preloadMinimal( obs, localeOpts, callback ) {
    ...
    ESModel.fetchBelongsTo( withTaxa, Taxon, taxonOpts, err => {
      ...
      TaxaController.assignAncestors( { }, taxa, { localeOpts, ancestors: true }, errr => {
