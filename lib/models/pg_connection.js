...
const PgConnection = class PgConnection {
  ...
  async connect( customConfig = { } ) {
    ...
    const dbname = PgConnection.buildDbName( );
    const pgConfig = {
  ...
  async query( query, values = undefined ) {
    ...
    PgConnection.customLogger( query, queryTime, this.connectionLabel );
    return results;
  }

  replica( ) {
    if ( _.isEmpty( this.replicas ) ) {
      return this;
    }

    return _.sample( this.replicas );
  }
  ...
  static customLogger( query, hrRuntime, connectionLabel = "primary" ) {
    ...
    PgConnection.debug( `  ${chalk.cyan.bold( `[${connectionLabel}] (${runtime}ms)` )} ${chalk.blue.bold( query )}` );
  }
