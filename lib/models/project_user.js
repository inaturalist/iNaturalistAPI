...
const ProjectUser = class ProjectUser extends Model {
  static async usersTrustingProjectFor( projectIdsArg, obscurationSourceArg ) {
    ...
    const query = squel
      ...
      .join( "projects", null, "projects.id = project_users.project_id" )
      .join( "preferences pp", null, "projects.id = pp.owner_id AND pp.owner_type = 'Project'" )
  ...
  static async projectsTrustedByUserFor( userId, obscurationSourceArg ) {
    ...
    const query = squel.select( )
      ...
      .where( "preferences.value IN ?", obscurationSource );
