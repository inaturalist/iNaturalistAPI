...
const ObservationPreload = class ObservationPreload {
  ...
  static async observationMentions( obs ) {
    if ( obs.length === 0 ) { return; }
    const obsIDs = _.map( obs, "id" );
    if ( _.isEmpty( obsIDs ) ) { return; }
    const query = squel.select( ).fields( [
      ...
      .from( "observation_mentions" )
      .where( "observation_id IN ?", obsIDs );
    const { rows } = await pgClient.query( query.toString( ) );
    await ESModel.fetchBelongsTo( rows, User, { } );
    const obsMentions = { };
    _.each( rows, r => {
      obsMentions[r.observation_id] = obsMentions[r.observation_id] || [];
      obsMentions[r.observation_id].push( r );
    } );
    _.each( obs, o => {
      o.observation_mentions = obsMentions[o.id] || [];
    } );
  }
};
