...
const ObservationPreload = class ObservationPreload {
  ...
  // Hydrate the `additional_observers` array (user objects) from the
  // `additional_observer_ids` integer array on each observation, mirroring how
  // other user references are loaded via ESModel.
  static async additionalObservers( obs ) {
    const allIds = _.uniq( _.compact( _.flattenDeep(
      _.map( obs, o => o.additional_observer_ids )
    ) ) );
    if ( _.isEmpty( allIds ) ) {
      _.each( obs, o => { o.additional_observers = []; } );
      return;
    }
    const usersHash = await ESModel.fetchResultsHashByIDs( allIds, User, { } );
    _.each( obs, o => {
      o.additional_observers = _.compact( _.map( o.additional_observer_ids || [], id => {
        const u = usersHash[id];
  ...
  static async observationPhotos( obs, options = { } ) {
    ...
    _.each( obs, o => {
      ...
      o.photos = _.filter( _.map( o.observation_photos, "photo" ), _.identity );
    } );
  ...
  static async assignPhotoFlags( photos ) {
    ...
    _.each( photos, p => {
      ...
      if ( _.some( p.flags, f => !f.resolved && ( f.flag === "copyright infringement"
        || f.flag === "artificially generated content" ) ) ) {
        if ( _.some( p.flags, f => !f.resolved && f.flag === "artificially generated content" ) ) {
          p.url = `${config.websiteURL}assets/artificially-generated-content-square.png`;
        } else {
  ...
  static async identifications( req, obs ) {
    if ( obs.length === 0 ) { return; }
    const obsIDs = _.map( obs, "id" );
    if ( _.isEmpty( obsIDs ) ) { return; }
    ...
    // TODO: fix the circular require issue
    // eslint-disable-next-line global-require
    const ExemplarIdentification = require( "./exemplar_identification" );
    await ESModel.fetchHasMany( identifications, ExemplarIdentification, "identification.id" );
    const exemplarIdentifications = [];
    _.each( obs, o => {
      _.each( o.identifications, i => {
        if ( i.identification ) {
          i.identification.exemplar_identification = _.first( i.exemplar_identifications ) || null;
          if ( i.identification.exemplar_identification ) {
            delete i.identification.exemplar_identification.identification;
            exemplarIdentifications.push( i.identification.exemplar_identification );
          }
        }
      } );
      o.identifications = _.filter( _.map( o.identifications, "identification" ), _.identity );
