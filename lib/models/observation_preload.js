...
const ObservationPreload = class ObservationPreload {
  ...
  static observationPhotosPromise( obs, resolve, reject ) {
    ...
    const query = squel.select( )
      ...
      .left_join( "flags", null, "photos.id = flags.flaggable_id AND flags.flag='copyright infringement' AND flags.resolved=false" )
    ...
    pgClient.connection.query( query.toString( ), ( err, result ) => {
      ...
      _.each( result.rows, r => {
        ...
        const photo = {
          ...
          url: r.flag
            ? `${config.websiteURL}assets/copyright-infringement-square.png`
            : util.fixHttps( r.square_url )
        };
