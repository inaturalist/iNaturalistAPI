...
const ObservationPreload = class ObservationPreload {
  ...
  static async observationPhotos( obs ) {
    ...
    const photoIDs = _.flatten( _.uniq( _.map( _.flatten( _.values( obsPhotos ) ), "photo_id" ) ) );
    ...
    if ( photoUserIDs.length > 0 ) {
      ...
      const { rows: photoUserRows } = await pgClient.query( photoUsersQuery.toString( ) );
    ...
    const { rows: flagRows } = await pgClient.query( flagsQuery.toString( ) );
    _.each( flagRows, r => {
    ...
    _.each( obs, o => {
      if ( _.isEmpty( obsPhotos[o.id] ) ) {
        ...
        o.photos = _.map( o.observation_photos, op => photos[op.photo_id] );
      }
    } );
  }
  ...
  static async observationSounds( obs ) {
    ...
    const flagsResult = await pgClient.query( flagsQuery.toString( ) );
    const soundFlags = {};
