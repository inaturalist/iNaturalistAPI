...
ObservationQueryBuilder.reqToElasticQueryComponents = async req => {
  ...
  if ( params.place_id && params.place_id !== "any" ) {
    ...
    if ( req.userSession ) {
      ...
      if (
        // Special attribute set when we are getting an obs query from
        // collection project search params. See projectRulesQueryFilters
        req._collectionProject
        && req.userSession.curated_project_ids
        && req.userSession.curated_project_ids.indexOf( req._collectionProject.id ) >= 0
      ) {
        // If the current user curates a specified collection project, they
        // should also see observations in that project by project members who
        ...
        if ( usersTrustingProjectForAny.length > 0 ) {
          placeFilterForUser.bool.should.push( {
            bool: {
              must: [
                privatePlaceFilter,
                { terms: { "user.id": usersTrustingProjectForAny } }
              ]
            }
          } );
        }
        ...
        if ( usersTrustingProjectForTaxon.length > 0 ) {
          placeFilterForUser.bool.should.push( {
            bool: {
              must: [
                privatePlaceFilter,
                { terms: { "user.id": usersTrustingProjectForTaxon } },
