...
ObservationQueryBuilder.applyLookupRules = async req => {
  ...
  if ( req.query.observation_accuracy_experiment_id ) {
    await ObservationQueryBuilder.applyObservationAccuracyExperimentFilter( req );
  }
};
...
ObservationQueryBuilder.reqToElasticQueryComponents = async req => {
  ...
  // When include_additional_observers is set and the user was supplied as a
  // login string, resolve it to a numeric ID so the additional_observer_ids
  // integer-array field can be matched via the bool.should OR filter.
  const includeAdditionalObservers = ( params.include_additional_observers === true
    || params.include_additional_observers === "true" );
  if ( includeAdditionalObservers && stringUserIDs.length > 0 ) {
    const resolvedLogins = [];
    await util.forEachInSequence( stringUserIDs, async login => {
      const lookedUpUser = await User.findByLogin( login );
      if ( lookedUpUser ) {
        numericUserIDs.push( lookedUpUser.id );
        resolvedLogins.push( login );
      }
    } );
    if ( resolvedLogins.length > 0 ) {
      params.user_id = numericUserIDs.join( "," );
      const remainingLogins = stringUserIDs.filter( l => !resolvedLogins.includes( l ) );
      params.user_login = remainingLogins.join( "," );
    }
  }
  ...
  _.each( [{ http_param: "rank", es_field: "taxon.rank" },
    {
      http_param: "user_id",
      // When include_additional_observers is set, match observations the user
      // either created or was added to as an additional observer. The array
      // es_field is expanded into a bool.should OR below. Accept the boolean
      // true (v2, Joi-coerced) or the string "true" (v1 raw query param).
      es_field: ( params.include_additional_observers === true
        || params.include_additional_observers === "true" )
        ? ["user.id", "additional_observer_ids"]
        : "user.id.keyword"
    },
    { http_param: "user_login", es_field: "user.login" },
    { http_param: "hour", es_field: "observed_on_details.hour" },
    ...
    { http_param: "created_day", es_field: "created_at_details.day" },
    { http_param: "created_month", es_field: "created_at_details.month" },
    { http_param: "created_year", es_field: "created_at_details.year" },
    { http_param: "created_week", es_field: "created_at_details.week" },
    ...
    if ( params[filter.http_param] && params[filter.http_param] !== "any" ) {
      ...
      // if the param is hour, require that the time_observed field is also populated
      if ( filter.http_param === "hour" ) {
        searchFilters.push( {
          exists: {
    ...
    const fieldFilter = {
      nested: {
        ...
        query: {
          bool: {
            filter: [
              esClient.termFilter( "dqa_stats.metric", dqaMetric ),
              esClient.termFilter( "dqa_stats.fail", true )
            ]
          }
        }
      }
    };
    const httpParam = `fails_dqa_${dqaMetric}`;
    if ( params[httpParam] === "true" ) {
      searchFilters.push( fieldFilter );
    } else if ( params[httpParam] === "false" ) {
      inverseFilters.push( fieldFilter );
    }
  } );
  ...
  if ( params.expected_nearby === "true" ) {
    searchFilters.push( {
      range: {
        geo_score: {
          gte: 1.0
  ...
  } else if ( params.expected_nearby === "false" ) {
    searchFilters.push( {
      range: {
        geo_score: {
          lt: 1.0
  ...
  if ( params.nelat
    ...
    || params.swlat
    || params.swlng
    || _.isNumber( params.nelat )
    || _.isNumber( params.nelng )
    || _.isNumber( params.swlat )
    || _.isNumber( params.swlng )
  ) {
    searchFilters.push( {
      envelope: {
        geojson: {
  ...
  if ( ( _.isNumber( params.lat ) || params.lat )
    && ( _.isNumber( params.lng ) || params.lng )
  ) {
    searchFilters.push( {
      geo_distance: {
    ...
    const nestedQuery = {
      nested: {
        ...
        query: {
          bool: {
            filter: [
              { range: { "annotations.vote_score_short": { gte: params.annotation_min_score } } }
            ]
          }
        }
      }
  ...
  if (
    ...
    const blocks = await req.userSession.getBlocks( );
    if ( !_.isEmpty( blocks ) ) {
      let usersToFilter = blocks.blockedUsers;
      if ( params.user_id ) {
      ...
      usersToFilter = usersToFilter.concat( blocks.blockedByUsers );
      inverseFilters.push(
  ...
  // merge the simple-parsed parameters with the originals. If a parameter was malformed by the
  // extended parser, its value with be an object and it will be skipped in the block below. Queries
  // made via V2 using POST and X-HTTP-Method-Override=GET will not have a `req._parsedUrl`, so
  // we must merge the simple-parsed parameters with the original
  _.each( { ...params, ...simpleParsedParams }, ( v, k ) => {
    ...
    // parameters that included escaped brackets will have been turned into objects by the
    // extended query parser. Skip those versions of the parameters, so the versions added by
    // the simple query parser above will be used instead
  ...
  if ( params.without_field ) {
    const fields = util.paramArray( params.without_field );
    inverseFilters.push( {
      nested: {
        path: "ofvs",
        query: {
  ...
  _.each( ["geoprivacy", "taxon_geoprivacy"], geoprivacyField => {
    ...
    _.each( util.paramArray( params[geoprivacyField] ), paramValue => {
      if ( paramValue === "obscured_private" ) {
        geoprivacyFieldFilters.push( esClient.termFilter( geoprivacyField, ["obscured", "private"] ) );
      } else if ( paramValue && paramValue !== "any" ) {
        const geoprivacyFieldFilter = esClient.termFilter(
          geoprivacyField, paramValue
        );
        if ( geoprivacyFieldFilter.terms[geoprivacyField].indexOf( "open" ) < 0 ) {
          geoprivacyFieldFilters.push( geoprivacyFieldFilter );
        } else {
    ...
    if ( geoprivacyFieldFilters.length === 1 ) {
      searchFilters = searchFilters.concat( geoprivacyFieldFilters );
    } else if ( geoprivacyFieldFilters.length > 1 ) {
      searchFilters.push( {
        bool: {
          should: geoprivacyFieldFilters
        }
      } );
    }
  } );
...
ObservationQueryBuilder.applyObservationAccuracyExperimentFilter = async req => {
  const experimentIDs = _.filter( _.map(
    util.paramArray( req.query.observation_accuracy_experiment_id ), Number
  ), _.identity );
  req.inat.observation_accuracy_experiment_id = experimentIDs;
  delete req.query.observation_accuracy_experiment_id;
  if ( _.isEmpty( experimentIDs ) ) {
    return;
  }
  const query = squel.select( ).field( "observation_id" )
    .from( "observation_accuracy_samples" )
    .where( "observation_accuracy_experiment_id IN ?", experimentIDs );
  ...
  if ( _.isEmpty( observationIDs ) ) {
    req.query.filters.push( { term: { id: -1 } } );
  } else {
    req.query.filters.push( { terms: { id: observationIDs } } );
  }
};
...
ObservationQueryBuilder.projectsQueryFilters = async ( projects, options = {} ) => {
  if ( options.userSession ) {
    ...
    // project pretty much at the same time, resulting in a lot of duplicated
    // queries. This loads blocks now so the data is cached and available for
  ...
  _.each( collectionsQueries, collectionQuery => {
    _.each( collectionQuery, ( value, parameter ) => {
      if ( _.isString( value ) ) {
        // ensure multivalued parameters are similarly ordered for comparison
