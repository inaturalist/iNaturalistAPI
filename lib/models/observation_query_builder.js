...
ObservationQueryBuilder.placeFilterForUser = async ( req, params ) => {
  ...
  const viewerTrustsProjectForAny = usersTrustingProjectForAny.includes(
    req.userSession.user_id
  );
  const viewerTrustsProjectForTaxon = usersTrustingProjectForTaxon.includes(
    req.userSession.user_id
  );
  // The overall intent here is
  // 1. To grant project curators access to obs by trusting users based on private coordinates
  // 2. To allow project members to see which observations are "in" or "out" of the project based on their trusting status
  ...
  if ( usersTrustingProjectForAny.length > 0 ) {
    ...
    if ( viewerCuratesProject ) {
      ...
      userFilterForAny = { terms: { "user.id": usersTrustingProjectForAny } };
      ...
      userFilterForAny = { term: { "user.id": req.userSession.user_id } };
    }
    if ( userFilterForAny ) {
  ...
  if ( usersTrustingProjectForTaxon.length > 0 ) {
    let userFilterForTaxon;
    // Viewer permissions largeley the same as above
    if ( viewerCuratesProject ) {
      userFilterForTaxon = { terms: { "user.id": usersTrustingProjectForTaxon } };
    } else if ( viewerTrustsProjectForTaxon ) {
      userFilterForTaxon = { term: { "user.id": req.userSession.user_id } };
    }
...
ObservationQueryBuilder.reqToElasticQueryComponents = async req => {
  ...
  if ( params.exact_taxon_id ) {
    searchFilters.push( esClient.termFilter(
      "taxon.id", params.exact_taxon_id
    ) );
  }
  ...
  if ( params.without_direct_taxon_id ) {
    inverseFilters.push( esClient.termFilter(
      "taxon.id", params.without_direct_taxon_id
    ) );
  }
  ...
  if ( params.coords_viewable_for_proj === "true" ) {
    ...
    const trustingUserFilter = esClient.termFilter( "user.id", trustingUserIds );
    ...
    const membersTrustingForTaxaFilter = {
      bool: {
        filter: [
          esClient.termFilter( "user.id", membersTrustingForTaxa ),
  ...
  if ( req.inat.without_ident_users ) {
    ...
    inverseFilters.push( {
      bool: {
        should: [
          esClient.termFilter( "identifier_user_ids", withoutIdentUserIDs )
        ]
      }
    } );
  }
  ...
  if ( req.userSession && req.userSession.blocks ) {
