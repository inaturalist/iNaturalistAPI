...
ObservationQueryBuilder.reqToElasticQueryComponents = async req => {
  ...
  // expansive expansive_term_value_id and expansive_without_term_value_id alternatives
  // consider documents outside the domain of the term
  if ( params.term_id ) {
    if ( params.expansive_term_value_id || params.expansive_without_term_value_id ) {
      const totalQuery = {
          bool: {
          ...
          must_not: [
            {
              nested: {
                ...
                query: {
                  bool: {
                    filter: { exists: { field: "annotations" } }
                  }
                }
              }
            }
      ...
      if ( params.expansive_term_value_id ) {
        const withSpecifiedTermValue = {
          nested: {
            path: "annotations",
            query: {
              bool: {
                should: [
                  {
                    bool: {
                      must: [
                        ...
                        esClient.termFilter( "annotations.controlled_value_id.keyword", params.expansive_term_value_id ),
        ...
        totalQuery.bool.should.push( withSpecifiedTermValue );
      ...
      } else if ( params.expansive_without_term_value_id ) {
        const withoutSpecifedTermValue = {
          nested: {
            path: "annotations",
            query: {
              bool: {
                must_not: [
                  {
                    bool: {
                      must: [
                        esClient.termFilter( "annotations.controlled_attribute_id.keyword", params.term_id ),
                        esClient.termFilter( "annotations.controlled_value_id.keyword", params.expansive_without_term_value_id ),
                        { range: { "annotations.vote_score": { gte: 0 } } }
                      ]
                    }
                  }
        ...
        totalQuery.bool.should.push( withoutSpecifedTermValue );
        const withoutFilters = [];
        withoutFilters.push(
          esClient.termFilter( "annotations.controlled_attribute_id.keyword", params.term_id )
        );
        ...
        withoutFilters.push( { range: { "annotations.vote_score": { gte: 0 } } } );
        inverseFilters.push( {
          nested: {
            path: "annotations",
            query: {
      ...
      totalQuery.bool.should.push( noAnnotations );
      searchFilters.push( totalQuery );
    } else {
