...
const HTTP_TIMEOUT_MS = Number( process.env.LANG_HTTP_TIMEOUT_MS || 1500 );
...
class LangService {
  ...
  static async translate( body, targetLocale, sourceLocale = null ) {
    ...
    if ( !body || !body.trim() || !targetLocale ) return body;
    try {
      const payload = { text: body, target_locale: targetLocale };
      if ( sourceLocale ) payload.source_locale = sourceLocale;
      const data = await postJSON( TRANSLATE_URL, payload );
      const translated = data?.text || data?.translation || data?.translated_text;
