...
const HTTP_TIMEOUT_MS = Number( process.env.LANG_HTTP_TIMEOUT_MS || 30000 );
...
class LangService {
  ...
  static async detect( body ) {
    console.log( `# detect # body = ${body?.slice?.( 0, 60 )} ..........` );
    if ( !body || !body.trim() ) return null;
    try {
      const data = await postJSON( DETECT_URL, { text: body } );
      const confidence = data?.confidence;
      const detectedLanguage = data?.language;
      console.log( `# detect # body = ${body?.slice?.( 0, 60 )}, confidence = ${confidence}, detectedLanguage = ${detectedLanguage}` );
      if ( confidence && confidence > 0.20 && detectedLanguage )
  ...
  static async translate( body, targetLocale, sourceLocale = null ) {
    console.log( `# translate # body = ${body?.slice?.( 0, 60 )}, sourceLocale = ${sourceLocale}, targetLocale = ${targetLocale} ..........` );
    ...
    try {
      ...
      console.log( `# translate # body = ${body?.slice?.( 0, 60 )}, sourceLocale = ${sourceLocale}, targetLocale = ${targetLocale}, translated = ${translated?.slice?.( 0, 60 )}` );
