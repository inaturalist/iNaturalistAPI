...
const Logstasher = class Logstasher {
  ...
  static afterRequestPayload( req, res, duration ) {
    ...
    if ( req.body && ( req._logClientError || req._logClientMessage ) ) {
      // I'd prefer the following to be real, indexed fields we can search and
      // aggregate on, but this works for now
      payload.qry = payload.qry || {};
      payload.qry.context = req.body.context;
      payload.qry.level = req.body.level;
