...
const Logstasher = class Logstasher {
  ...
  static beforeRequestPayload( req ) {
    ...
    payload.body_string = _.isEmpty( req.body ) ? null : JSON.stringify( _.omit( req.body, "fields" ) );
    ...
    payload.fingerprint = req.headers.fingerprint || "";
  ...
  static writeFeatureFlagsUpstreamErrorLog( err ) {
    if ( !Logstasher.logWriteStream( ) ) { return; }
    const payload = {
      "@timestamp": new Date( ).toISOString( ),
      subtype: "FeatureFlagsUpstreamError",
      // inaturalistjs/fetch errors carry the upstream HTTP status on err.response
      status: ( err && err.response && err.response.status ) || ( err && err.status ) || 500,
...
Logstasher.headersToStash = [
  ...
  "via", "x-via", "x-country-code", "x-installation-id", "x-http-method-override"
];
