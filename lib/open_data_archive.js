...
const OpenDataArchive = class OpenDataArchive {
  ...
  async uploadGzippedArchive( ) {
    ...
    await S3client.send( new CopyObjectCommand( {
      ...
      CopySource: `${config.aws.openDataBucket}/metadata/${this.outputDirName}.tar.gz`,
      ACL: config.aws.openDataACL || "public-read",
      ContentEncoding: "gzip"
    } ) );
  }
  ...
  async uploadGzippedFiles( ) {
    ...
    await Promise.all( _.map( files, file => this.uploadGzippedFile( `${file}.csv`, `${file}.csv.gz` ) ) );
  }
  ...
  gzipArchive( ) {
    return tar.c( { gzip: true, file: `${this.outputDir}.tar.gz` }, [this.outputDir] );
  }
  ...
  async photosProcessor( beginningID, startID, maxPhotoID ) {
    ...
    const photoRegex = new RegExp(
      `${_.escapeRegExp( PHOTO_DOMAIN )}\\/photos\\/[0-9]+\\/original\\.(.*)(\\?|$)`
    );
    ...
    _.each( rows, row => {
      if ( row.original_url ) {
        ...
        let matches = row.original_url.match( /^(.*)\?[0-9]+$/ );
        if ( matches ) {
        ...
        matches = originalURL.match( photoRegex );
        if ( matches ) {
          const extension = matches[1];
          if ( VALID_EXTENSIONS[_.toLower( extension )] ) {
