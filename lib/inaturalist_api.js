...
InaturalistAPI.leafTaxaCounts = function( req, callback ) {
  InaturalistAPI.leafCounts( req, function( err, leafCounts ) {
    ...
    var source = [ "id", "name", "names", "rank", "rank_level",
      "default_photo", "is_active", "iconic_taxon_id",
      "ancestor_name", "ancestor_rank", "listed_taxa", "statuses" ];
    var searchReq = { query: { is_active : null, size: 500 }, _source: source };
    InaturalistAPI.taxaSearchQuery(searchReq, null, filters, function( err, response ) {
      ...
      _.each( response.results, function( t ) {
        t.names = _.filter( t.names, function( n ) {
          return n.locale == "en" || n.locale == req.query.locale;
        });
        var reqPlaceID = parseInt( req.query.place_id );
        if( reqPlaceID ) {
          t.listed_taxa = _.filter( t.listed_taxa, function( lt ) {
            return lt.place_id === reqPlaceID;
          });
        } else { t.listed_taxa = [ ]; }
        t.statuses = _.filter( t.statuses, function( s ) {
          return !s.place_id || s.place_id === reqPlaceID;
        });
...
InaturalistAPI.taxaSearchQuery = function( req, search_wheres, search_filters, callback ) {
  ...
  esClient.connection.search({
    ...
    _source: req._source || [ "id", "name", "names", "rank", "default_photo_url",
...
InaturalistAPI.placesNearby = function( req, callback ) {
  var placesQuery = _.extend( { }, req );
  // looking up standard places
  placesQuery.query.community = false;
  InaturalistAPI.placesNearbyQuery( placesQuery,
    function( err, response ) {
      if( err ) { return callback( err ); }
      var standardPlaces = response.results || [ ];

      // looking up comunity places
      placesQuery.query.community = true;
      InaturalistAPI.placesNearbyQuery( placesQuery,
        function( err, response ) {
          if( err ) { return callback( err ); }
