...
InaturalistAPI.validateSession = function( req, res, next ) {
  ...
  if ( req.headers.authorization ) {
    ...
    jwt.verify( token, config.jwtSecret || "secret",
      ...
      InaturalistAPI.addUserInfoToSession( req, next );
    } );
  } else {
    next( );
  }
...
InaturalistAPI.addUserInfoToSession = function( req, callback ) {
  ...
  User.projectsCurated( req.userSession.user_id, ( err, projectIDs ) => {
    ...
    User.localeDefaults( req.userSession.user_id, ( err, defaults ) => {
      ...
      User.blocks( req.userSession.user_id, ( err, blocks ) => {
        if ( blocks ) {
          req.userSession = Object.assign( { }, req.userSession, {
            blocks
          } );
        }
        callback( );
...
InaturalistAPI.server = function( logWriteStream ) {
  ...
  app.get( "/v1/tiles.json", ( req, res ) => {
    let urlBase = `${config.host}/v1/colored_heatmap/{z}/{x}/{y}.torque.json`;
  ...
  app.get( "/v1/taxa/:id/wanted", routesV1.taxa_wanted );
  ...
  app.get( "/v1/identifications/species_counts", routesV1.identifications_species_counts );
