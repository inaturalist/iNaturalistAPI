...
InaturalistAPI.server = function( ) {
  ...
  dfault( "put", "/v1/observation_photos/:id", ObservationPhotosController.update, { routeHandler: upload.fields( [
    { name: "file", maxCount: 1 }
  ] ) } );
...
InaturalistAPI.lookupIdentifierUsersMiddleware = function( req, res, next ) {
  if ( req.method === "OPTIONS" ) { return next( ); }
  if( !req.query.ident_user_id ) { return next( ); }
  InaturalistAPI.lookupInstancesMiddleware(
    req, res, next, "ident_user_id", User.findAllByLoginOrID, "ident_users" );
};
