...
InaturalistAPI.validateSession = function( req, res, next ) {
  if ( req.headers.authorization ) {
    jwt.verify( req.headers.authorization, config.jwtSecret || "secret",
      ...
      InaturalistAPI.addCuratedProjectIDsToSession( req, next );
    } );
  } else {
    next( );
  }
...
InaturalistAPI.addCuratedProjectIDsToSession = function( req, callback ) {
...
InaturalistAPI.server = function( ) {
  ...
  if ( config.imageProcesing && config.imageProcesing.tensorappURL &&
    ...
    app.post( "/v1/computervision/score_image", upload.single( "image" ), ( req, res ) => {
      ComputervisionController.scoreImage( req, ( err, data ) => {
