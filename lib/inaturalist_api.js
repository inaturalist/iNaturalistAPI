...
InaturalistAPI.reqToElasticQuery = function( req, callback ) {
  ...
  if( req.inat && req.inat.apply_project_rules_for ) {
    req.inat.apply_project_rules_for.searchParams( function( err, r ) {
      delete params.apply_project_rules_for;
      delete req.inat.apply_project_rules_for;
      params = _.extend( { }, params, r );
      // the project might have a list rule. Set the list so we
      // can fetch its taxon_ids next iteration
      if( r.list ) {
        req.inat.list = r.list;
      }
      InaturalistAPI.reqToElasticQuery( _.extend( { }, req, { query: params }), callback );
    });
    return;
  }
  ...
  if( params.verifiable == "true" ) {
    search_filters.push(
      esClient.termFilter( "quality_grade", [ "research", "needs_id" ] ) );
  ...
  // query parser
  simpleParsedParams = req._parsedUrl ? querystring.parse( req._parsedUrl.query ) : { };
  ...
  req.query.aggs = {
    ids: {
      ...
      aggs: {
        ...
        top_identifiers: {
          terms: { field: "non_owner_ids.user.id", size: 500 }
        }
      }
    }
  };
...
InaturalistAPI.lookupProjectRulesMiddleware = function( req, res, next ) {
  InaturalistAPI.lookupInstanceMiddleware(
    req, res, next, "apply_project_rules_for", Project.findByID, "apply_project_rules_for" );
};
...
InaturalistAPI.lookupInstance = function( req, paramKey, method, callback ) {
  if( req.query[ paramKey ] ) {
    ...
    ids = util.paramArray( req.query[ paramKey ] );
