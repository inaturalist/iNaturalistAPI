...
InaturalistAPI.server = function( ) {
  ...
  var CommentsController = require( "./controllers/v1/comments_controller" ),
      ...
      TestvisionController = require( "./controllers/v1/testvision_controller"),
  ...
  var dfaultHTML = ( action, path, method ) => {
    app[action]( path, ( req, res ) => {
      routesV1.defaultHTML( method, req, res );
    });
  };
  ...
  if ( config.imageProcesing && config.imageProcesing.url && config.imageProcesing.uploadsDir ) {
    ...
    app.post( "/testuploads", upload.single( "testimage" ), ( req, res ) => {
      TaxaController.similarToImage( req, ( err, data ) => {
        util.renderJSONPresponse( req, res, err, data );
      });
    });
    dfaultHTML( "get", "/testvision/:id", TestvisionController.compare );
  }
