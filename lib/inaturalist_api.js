...
InaturalistAPI.preloadAndProcess = ( method, req, res, opts = { } ) => {
  ...
  if ( req.userSession && req.userSession.user_id && !_.isEmpty( options.preload ) ) {
    ...
    if ( options.preload.userPrivileges ) {
      User.userPrivileges( req.userSession.user_id, ( err, privileges ) => {
        if ( privileges ) {
          req.userSession.userPrivileges = privileges;
        }

        delete options.preload.userPrivileges;
        InaturalistAPI.preloadAndProcess( method, req, res, options );
      } );
      return;
    }
...
InaturalistAPI.server = ( ) => {
  ...
  dfault( "get", "/v1/observations/:id", ObservationsController.show, {
    ...
    defaultTTL: -1,
    preload: Object.assign( {}, allPreloads, { trustingUsers: true, userPrivileges: true } )
  } );
