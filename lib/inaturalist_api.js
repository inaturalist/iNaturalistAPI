...
InaturalistAPI.elasticResults = function( req, index, callback ) {
  req.elastic_query = InaturalistAPI.reqToElasticQuery( req );
...
InaturalistAPI.reqToElasticQuery = function( req ) {
  var p = req.query;
  var search_wheres = [ ];
  var extra_preloads = [ ];
  var search_filters = [ ];
...
InaturalistAPI.lookupNotInProjectMiddleware = function( req, res, next ) {
  InaturalistAPI.lookupInstanceMiddleware(
    req, res, next, "not_in_project", Project.findByID, "project" );
};
