...
InaturalistAPI.server = async ( ) => {
  ...
  const elasticMapperConfig = _.extend( { }, config, {
  ...
  elasticMapperConfig.database.dbname = pgClient.buildDbName( );
  elasticMapperConfig.elasticsearch.searchIndex = esClient.buildIndexName( "observations" );
  const app = ElasticMapper.server( elasticMapperConfig );
