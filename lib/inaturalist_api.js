...
InaturalistAPI.leafCount = function( req, callback ) {
  ...
  InaturalistAPI.elasticResults( req, "observations", function( err, data ) {
    ...
    leaf_ids = _.keys( _.pick( ancestorCounts, function( value, key ) {
      return value == 0;
    }));
    callback(null, { leaf_count: leaf_ids.length });
  });
};

InaturalistAPI.speciesCounts = function( req, callback ) {
  InaturalistAPI.leafCount( req, function( err, leafCount ) {
    if( err ) { return callback( err ); }
    callback(null, leafCount);
  });
};
