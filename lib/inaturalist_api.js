...
InaturalistAPI.prepareApp = ( ) => {
  const server = express( );

  // since https://github.com/ljharb/qs/blob/main/CHANGELOG.md#6141, the default arrayLimit of
  // 20 for `qs` is actually being enforced. This codebase expected to be able to handle more
  // than 20. Explicitly configuring the query parser to expand on the express `extended` default
  // by setting a higher `arrayLimit` allows for more array[]= parameters since qs 6.14.1
  server.set( "query parser", str => (
    qs.parse( str, {
  ...
  server.use( util.accessControlHeaders );
  ...
  server.use( InaturalistAPI.replaceUserLoginMiddleware );
...
InaturalistAPI.server = async ( ) => {
  ...
  const app = InaturalistAPI.prepareApp( );
  ...
  const TaxonNamePrioritiesController = require( "./controllers/v1/taxon_name_priorities_controller" );
  ...
  // map tile routes
  app.get( "/v1/:style/:zoom/:x/:y.:format([a-z.]+)", InaturalistMapserver.defaultRoute );
  ...
  app.get( "/v1/geomodel/:taxon_id/:zoom/:x/:y.:format([a-z.]+)",
    InaturalistMapserver.geomodelRoute );
  app.get( "/v1/geomodel_taxon_range/:taxon_id/:zoom/:x/:y.:format([a-z.]+)",
    InaturalistMapserver.geomodelTaxonRangeRoute );
  app.get( "/v1/geomodel_comparison/:taxon_id/:zoom/:x/:y.:format([a-z.]+)",
    InaturalistMapserver.geomodelComparisonRoute );
  ...
  dfault( "get", "/v1/taxa/iconic", TaxaController.iconic );
  ...
  dfault( "get", "/v1/users/email_available", UsersController.emailAvailable );
  dfault( "get", "/v1/users/me", UsersController.me );
  dfault( "get", "/v1/users/notification_counts", UsersController.notificationCounts );
  dfault( "get", "/v1/users/recent_observation_fields", UsersController.recentObservationFields );
  ...
  // Taxon Name Priorities
  dfault( "post", "/v1/taxon_name_priorities", TaxonNamePrioritiesController.create );
  dfault( "put", "/v1/taxon_name_priorities/:id", TaxonNamePrioritiesController.update );
  dfault( "delete", "/v1/taxon_name_priorities/:id", TaxonNamePrioritiesController.delete );
...
InaturalistAPI.setRequestPaginationData = ( req, options ) => {
  const { page, perPage } = InaturalistAPI.paginationData( req, options );
  req.query.page = page;
...
InaturalistAPI.iNatJSWrap = async ( method, req, opts = {} ) => {
  const options = { ...opts };
  ...
  options.api_token = req.headers.authorization;
  options.user_agent = req.headers["user-agent"];
  options.remote_ip = req.headers["x-forwarded-for"];
  if ( req.headers["x-installation-id"] ) {
    options.headers["X-Installation-ID"] = req.headers["x-installation-id"];
  }
