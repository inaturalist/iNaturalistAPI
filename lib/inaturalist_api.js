...
InaturalistAPI.validateSessionAsync = async ( req, res, next ) => {
  ...
  } catch { return void next( ); }
  if ( !_.isObject( jwtPayload ) ) { return void next( ); }
  req.userSession = jwtPayload;
...
InaturalistAPI.validateApplicationAsync = async ( req, res, next ) => {
  ...
  try {
    jwtPayload = jwt.verify( req.headers.authorization,
      config.jwtApplicationSecret || "application_secret", { algorithms: ["HS512"] } );
  } catch { return void next( ); }
  if ( !_.isObject( jwtPayload ) ) { return void next( ); }
...
InaturalistAPI.server = ( ) => {
  ...
  const allPreloads = { userBlocks: true, curatedProjects: true };
  // Observations
  ...
  app.post( "/v1/taxa/suggest", upload.single( "image" ), ( req, res ) => {
    InaturalistAPI.preloadAndProcess( TaxaController.suggest, req, res, {
      async: true
    } );
  } );
