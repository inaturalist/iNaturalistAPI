...
ElasticMapper.DEFAULT_OBSERVATION_MAP_FIELDS = [
...
ElasticMapper.csvFromResult = async ( req, result ) => {
  ...
  if ( result.aggregations && result.aggregations.zoom1 ) {
    target = result.aggregations.zoom1.buckets;
    const observationIDs = _.compact( _.map(
      result.aggregations.zoom1.buckets, b => b?.callMaxObsID?.value
    ) );
    if ( !_.isEmpty( observationIDs ) ) {
      ...
      const geohashObs = await ESModel.mgetResults( observationIDs, "observations", {
        source: util.sourceParams( ElasticMapper.DEFAULT_OBSERVATION_MAP_FIELDS )
      } );
  ...
  const csvData = _.map( target, hit => {
    ...
    const fieldData = hit._source || tileObservationsByID[hit.callMaxObsID.value];
    fieldData.private_location = !_.isEmpty( fieldData.private_location );
    if ( !hit._source && hit.callMaxObsID ) {
      fieldData.cellCount = hit.doc_count;
    }
    const properties = { };
    let value;
