...
const AnnouncementsController = class AnnouncementsController {
  static async search( req ) {
    let query = squel.select( )
      .field( "announcements.id, body, placement, dismissible, start, \"end\"" )
      .from( "announcements" )
    ...
    // locale filter
    if ( queryLocale ) {
      const emptyLocaleClause = squel.expr( ).or( "locales IS NULL OR locales = '{}'" );
      // first attempt to fetch announcements with an exact match for locale
      const exactLocaleQuery = query.clone( ).where( emptyLocaleClause
        .clone( ).or( "? = ANY( locales )", queryLocale ) );
      ...
      if ( _.isEmpty( announcementRows ) && queryLocale.match( "-" ) ) {
        const baseLocaleQuery = query.clone( ).where( emptyLocaleClause
          .clone( ).or( "? = ANY( locales )", queryLocale.split( "-" )[0] ) );
