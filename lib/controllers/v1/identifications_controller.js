...
var IdentificationsController = class IdentificationsController {
  ...
  static recentTaxa( req, callback ) {
    ...
    IdentificationsController.elasticResults( countReq, ( err, data ) => {
      ...
      let bucketsPage = buckets.slice( start, start + perPage );
      ESModel.fetchBelongsTo( bucketsPage, Identification, { }, function( err ) {
        if( err ) { return callback( err ); }
        ESModel.fetchBelongsTo( bucketsPage, Taxon, taxonOpts, function( err ) {
          if( err ) { return callback( err ); }
          bucketsPage = _.reject( bucketsPage, b => {
            if( !b.taxon ) { return true; }
            if( req.query.taxon_id &&
                !_.includes( b.taxon.ancestor_ids, req.query.taxon_id ) ) {
              return true;
            }
          });
          callback( null, {
            ...
            results: _.sortBy( bucketsPage, function( b ) {
