...
const IdentificationsController = class IdentificationsController {
  static async create( req ) {
    ...
    await Identification.preloadInto( req, arr, localeOpts );
  ...
  static async search( req ) {
    ...
    await Promise.all( [
      Observation.preloadInto( req, response.results, localeOpts ),
      ESModel.fetchBelongsTo( req, response.results, User )
    ] );
    return response;
  }
  ...
  static reqToElasticQuery( req ) {
    ...
    inverseFilters.push( esClient.termFilter( "hidden", true ) );
    ...
    _.each( [
      ...
      { http_param: "taxon_change_id", es_field: "taxon_change.id" }
    ], filter => {
