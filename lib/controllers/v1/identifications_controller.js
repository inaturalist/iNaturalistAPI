...
var IdentificationsController = class IdentificationsController {
  ...
  static reqToElasticQuery( req, callback ) {
    ...
    _.each([
      ...
      { http_param: "created_at", es_field: "created_at" },
  ...
  static relatedSpecies( req, callback ) {
    ...
    Taxon.findByID( req.query.taxon_id, ( err, taxon ) => {
      ...
      IdentificationsController.speciesCounts( { query: idQuery }, ( err, rsp1 ) => {
        ...
        IdentificationsController.speciesCounts( { query: idQuery }, ( err, rsp2 ) => {
          if( err ) { return callback( err ); }
  ...
  static newTaxa( req, callback ) {
    ...
    countQuery.aggs = {
      total: { cardinality: { field: "taxon.ancestors.id", precision_threshold: 10000 } },
      taxa: {
        terms: {
          field: "taxon.ancestors.id",
    ...
    IdentificationsController.elasticResults( countReq, ( err, data ) => {
      ...
      var buckets = _.map( data.aggregations.taxa.buckets, function( b ) {
        return { taxon_id: b.key, created_at: b.earliest.value_as_string };
      });
      var localeOpts = util.localeOpts( req );
      ...
      ESModel.fetchBelongsTo( buckets, Taxon, taxonOpts, function( err ) {
        if( err ) { return callback( err ); }
        buckets = _.reject( buckets, b => !b.taxon );
        callback( null, {
