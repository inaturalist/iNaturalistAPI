...
const IdentificationsController = class IdentificationsController {
  ...
  static elasticResults( req, callback ) {
    IdentificationsController.reqToElasticQuery( req, ( err, query ) => {
      if ( err ) { return void callback( err ); }
      ESModel.elasticResults( req, query, "identifications", { track_total_hits: true }, callback );
    } );
  }
    ...
    IdentificationsController.elasticResults( countReq, ( err, data ) => {
      ...
      ESModel.fetchBelongsTo( recentLeaves, Taxon, leafTaxonOpts, errr => {
        ...
        _.each( recentLeaves, leaf => {
          if ( !branchAncestryPrefix || leaf.taxon.min_species_ancestry.match( branchAncestryPrefix ) ) {
