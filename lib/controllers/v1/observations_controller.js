...
ObservationsController.elasticResults = async function observationElasticResults( req ) {
  ...
  const noLookupParms = _.isEmpty( _.without( _.keys( req.inat ), "preferredPlace" ) );
  if ( fetchingOnlyIDs && noLookupParms
    && _.isEmpty( query.inverse_filters )
    && _.isEmpty( query.grouped_inverse_filters )
    && _.isEqual( query.sort, { created_at: "desc" } )
  ) {
    const sourceParams = returnOnlyID ? ["id"] : util.sourceParams( {
      includes: opts.includes,
      excludes: opts.excludes
    } );
    const getResponse = await esClient.connection.mget( Object.assign( {
      preference: global.config.elasticsearch.preference,
      index: `${process.env.NODE_ENV || global.config.environment}_observations`,
      body: { ids: fetchingOnlyIDs }
    }, util.sourceParams( sourceParams ) ) );
    return {
      hits: {
        total: {
          value: _.size( getResponse.docs )
        },
        hits: getResponse.docs
      }
    };
  }
