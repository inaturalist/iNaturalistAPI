...
const postProcessTranslate = async (results, req) => {
  try {
      ...
      if (Array.isArray(obs.comments)) {
        obs.comments = await Promise.all(obs.comments.map(async c => {
          try {
            ...
            const detectedLanguage = await LangUtils.detect(c.body);
            console.log("detectedLanguage =", detectedLanguage);

            if (!detectedLanguage || detectedLanguage === currentLocale) {
              return c;
            }

            console.log("Translate language...");
            const translatedBody = await LangUtils.translate(c.body, currentLocale, detectedLanguage);
            console.log("translatedBody =", translatedBody?.slice?.(0, 60));

            return {
              ...c,
              body: translatedBody || c.body,
              original_body: c.body,
              original_language: detectedLanguage
            };
          } catch (e) {
            console.error("comment translate error:", e?.message || e);
            return c;
          }
        }));
      }

      // Identifications (uncomment if/when you want this too)
      /*
      if (Array.isArray(obs.identifications)) {
        obs.identifications = await Promise.all(obs.identifications.map(async i => {
          try {
            const detectedLanguage = await LangUtils.detect(i.body);
            if (!detectedLanguage || detectedLanguage === currentLocale) {
              return i;
            }
            const translatedBody = await LangUtils.translate(i.body, currentLocale);
            return {
              ...i,
              body: translatedBody || i.body,
              original_body: i.body,
              original_language: detectedLanguage
            };
          } catch (e) {
            console.error("ident translate error:", e?.message || e);
            return i;
          }
        }));
      }
    ...
    console.log("postProcessTranslate top-level error:", err?.message || err);
    return results; // never throw from here
  }
};
...
ObservationsController.search = async ( req, options = { } ) => {
  ...
  try {
    const out = await postProcessTranslate(data, req);
    return out || data; // never return null/undefined
  } catch (err) {
    console.error("postProcessTranslate error:", err?.message || err);
    return data; // fail soft: never throw from here
  }
};
