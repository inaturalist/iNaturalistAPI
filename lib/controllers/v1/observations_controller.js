...
ObservationsController.newProjectsContaining = async ( obs, options = {} ) => {
  ...
  // lookup relevant, joined, collections
  ...
  const umbrellaProjectIDs = _.uniq( _.flatten( _.map(
  ...
  const trustedProjectsQuery = Object.assign( {}, collectionsQuery, {
    size: 100,
    _source: ["id"]
  } );
  ...
  if (
    ...
    if ( trustedProjectIDs.length > 0 ) {
      ...
      const trustedProjectsResult = await ESModel.elasticResults(
        { }, // req
        trustedProjectsQuery, // query
        "projects" // index
      );
      const observationTrustedProjectIDs = _.map( trustedProjectsResult.hits.hits, "_source.id" );
