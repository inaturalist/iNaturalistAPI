...
ObservationsController.histogram = function( req, callback ) {
  ...
  req.query.interval = InaturalistAPI.setDefaultParamValue(
    req.query.interval, "month_of_year", { enum:
      [ "year", "month", "week", "day", "month_of_year", "week_of_year", "day_of_month" ] } );
  ...
  if( grouped ) {
    ...
    countQuery.aggs = {
      histogram: { date_histogram: { field: queryField, interval: interval } }
    };
  }
  ...
  ObservationsController.elasticResults( countReq, "observations", function( err, data ) {
    ...
    } else if( req.query.interval === "day_of_month" ) {
      resultOptions.backfill = { min: 1, max: 31 };
    }
    var results = { };
