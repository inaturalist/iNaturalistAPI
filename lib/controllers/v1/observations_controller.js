...
ObservationsController.search = function( req, callback ) {
  ...
  ObservationsController.resultsForRequest( req, function( err, data ) {
    ...
    const preloadAndReturn = returnData => {
      preloadCallback( returnData.results, localeOpts, function( err ) {
        if( err ) { return callback( err ); }
        if ( !req.query.only_id ) {
...
ObservationsController.elasticResults = function observationElasticResults( req, callback ) {
  ObservationsController.reqToElasticQuery( req, function( err, query ) {
    if( err ) { return callback( err ); }
    var opts = { excludes: [ "taxon.names", "taxon.photos", "taxon.taxon_photos", "taxon.names_*" ] };
...
ObservationsController.reqToElasticQueryComponents = function( req, callback ) {
  ...
  if ( params.taxon_name ) {
    search_filters.push({ multi_match: {
      query: params.taxon_name, operator: "and", fields: [ "taxon.names.name", "taxon.names_*" ] } } );
  }
  ...
  _.each([ { http_param: "rank", es_field: "taxon.rank" },
    ...
    if( params[ filter.http_param ] && params[ filter.http_param ] != "any" ) {
      if( _.isArray( filter.es_field ) ) {
        search_filters.push( {
          bool: {
            should: _.map( filter.es_field, field =>
              esClient.termFilter( field, params[ filter.http_param ] )
            )
          }
        })
      } else {
  ...
  _.each([ { http_param: "photos", es_field: [ "photos.url", "photos_count" ] },
    ...
    if( params[ filter.http_param ] == "true" ) {
      search_filters.push( fieldFilter );
    } else if( params[ filter.http_param ] == "false" ) {
      inverse_filters.push( fieldFilter );
    }
  });
  ...
  if ( req.inat && req.inat.ident_users ) {
    const ident_user_ids = _.map( req.inat.ident_users, u => u["id"] );
    let nestedFilter;
    nestedFilter = esClient.termFilter( "identifications.user.id", ident_user_ids );
    search_filters.push( {
      bool: {
        should: [
          esClient.termFilter( "identifier_user_ids", ident_user_ids ),
          {
            nested: {
