...
ObservationsController.reqToElasticQueryComponents = function( req, callback ) {
  ...
  if ( params.place_id && params.place_id !== "any" ) {
    const placeFilter = { http_param: "place_id", es_field: "place_ids" };
    if ( req.userSession ) {
      const userLoginIsUser = params.user_login && req.userSession.user_login === params.user_login;
      const userIDIsUser = params.user_id && (
        req.userSession.user_id === params.user_id  || req.userSession.user_login === params.user_id
      );
      if ( userLoginIsUser || userIDIsUser ) {
        placeFilter.es_field = "private_place_ids";
      }
    }
    search_filters.push( esClient.termFilter( placeFilter.es_field, params[ placeFilter.http_param ] ) );
  }
