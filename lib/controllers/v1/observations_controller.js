...
ObservationsController.projectQueryFromObsSearchParams = ( obsSearchParams, options = { } ) => {
  const filters = [
...
ObservationsController.newProjectsContaining = async ( obs, options = {} ) => {
  ...
  if (
    ...
    && options.userSession
    && ( options.userSession.id === obs.user_id
      || (
    ...
    // else (obs obscured by taxon_geoprivacy)
      ...
      potentialTrustedProjectIDs = await ProjectUser.projectsTrustedByUserFor( obs.user.id, "taxon" );
    }
    ...
    if ( trustedProjectIDs.length > 0 ) {
      ...
      const {
        ...
        obsSearchParams, { privateLocation: true }
      );

      const trustedProjectsQuery = {
        filters: trustedFilters.concat( [
          esClient.termFilter( "id", trustedProjectIDs ),
