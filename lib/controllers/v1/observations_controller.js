...
ObservationsController.search = function( req, callback ) {
  ObservationsController.resultsForRequest( req, function( err, data ) {
    ...
    if( req.query.details === "all" ) {
      ...
      preloadCallback = Observation.preloadTaxaAndUsers;
    }
  ...
  if( req.query.term_id ) {
    nested_query = {
      nested: {
        ...
        query: { bool: { must: [
          { term: { "controlled_terms_test.controlled_attribute_id": req.query.term_id } },
    ...
    if( req.query.term_value_id ) {
      nested_query.nested.query.bool.must.push({
        match: { "controlled_terms_test.controlled_value_id": req.query.term_value_id } });
    }
...
ObservationsController.iconicTaxaCounts = function( req, callback ) {
  ...
  ObservationsController.elasticResults( countReq, "observations", function( err, data ) {
    ...
    ESModel.fetchBelongsTo( buckets, Taxon, prepareTaxon, function( err ) {
      if( err ) { return callback( err ); }
      callback( null, {
...
ObservationsController.iconicTaxaSpeciesCounts = function( req, callback ) {
  ...
  ObservationsController.elasticResults( countReq, "observations", function( err, data ) {
    ...
    var ESModel = require( "../../models/es_model" );
    ESModel.fetchBelongsTo( iconicTaxonLeafCounts, Taxon, prepareTaxon, function( err ) {
      if( err ) { return callback( err ); }
      callback( null, {
...
ObservationsController.updates = function( req, callback ) {
  ...
  InaturalistAPI.setPerPage( req, { default: 200, max: 200 } );
  esClient.connection.search({
    preference: global.config.elasticsearch.preference || "_local",
      ...
      query: {
        bool: {
          must: [
            ...
            { not: { term: { viewed_subscriber_ids: req.userSession.user_id } } }
          ]
        }
      },
    ...
    var hits = _.map( response.hits.hits, h => h._source );
...
ObservationsController.popularFieldValues = function( req, callback ) {
  ...
  countQuery.aggs = {
    terms: {
      nested: { path: "controlled_terms_test" },
      aggs: { popular_term_values: { terms: {
        field: "controlled_terms_test.attribute_value",
