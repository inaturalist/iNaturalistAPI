...
ObservationsController.loadNewProjects = async obs => (
  ...
  obs.reduce( ( promiseChain, item ) => (
    promiseChain.then( ( ) => new Promise( ( resolve, reject ) => {
      ObservationsController.newProjectsContaining( item ).then( projectIDs => {
...
ObservationsController.newProjectsContaining = async obs => {
  ...
  const data = await ESModel.elasticResultsAsync( { query: { } }, query, "projects" );
  ...
  const umbrellaData = await ESModel.elasticResultsAsync( { query: { } }, umbrellaProjectsQuery, "projects" );
...
ObservationsController.lifelistExport = async req => {
  ...
  await ESModel.fetchBelongsToAsync( taxaCounts, Taxon, taxonOpts );
  ...
  ESModel.fetchBelongsToAsync( ancestorCounts, Taxon, taxonOpts );
...
ObservationsController.umbrellaProjectStats = async req => {
  ...
  await ESModel.fetchBelongsToAsync( results, Project, { source: Project.returnFields } );
  await ESModel.fetchBelongsToAsync( _.map( results, "project" ), Place, placeOptions );
  return {
...
ObservationsController.qualityMetrics = async req => {
  ...
  const { rows } = await pgClient.connection.query( query.toString( ) );
  await ESModel.fetchBelongsToAsync( rows, User );
  return {
    total_results: rows.length,
    page: 1,
