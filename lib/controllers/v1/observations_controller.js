...
ObservationsController.histogram = function( req, callback ) {
  ...
  if( grouped ) {
    ...
    countQuery.aggs = {
      histogram: { terms: { field: `${queryField}_details.${interval}`, size: 0 } }
    };
  } else {
  ...
  if( req.query.term_id ) {
    nested_query = {
      nested: {
        path: "annotations",
        query: { bool: { must: [
          { term: { "annotations.controlled_attribute_id": req.query.term_id } },
          { range: { "annotations.vote_score": { gte: 0 } } }
        ] } }
      }
    }
    if( req.query.term_value_id ) {
      nested_query.nested.query.bool.must.push({
        match: { "annotations.controlled_value_id": req.query.term_value_id } });
    }
    search_wheres.push( nested_query );
  } else if( req.query.annotation_min_score || req.query.annotation_min_score === 0 ) {
    nested_query = {
      nested: {
        path: "annotations",
        query: { bool: { must: [
  ...
  if( params.changed_since ) {
    ...
    if( changedDate && changedDate.isValid( ) ) {
      ...
      if( params.changed_fields ) {
        ...
        nested_query.nested.query.filtered.query.bool.must.push({
...
ObservationsController.resultsForRequest = function( req, callback ) {
  ObservationsController.elasticResults( req, function( err, data ) {
    ...
    var obs = _.map( data.hits.hits, function( h ) {
      return new Observation( h._source, { session: req.userSession } );
    });
    if( err ) { return callback( err ); }
...
ObservationsController.updates = function( req, callback ) {
  ...
  if( req.query.viewed == "false" ) {
    updatesFilters.push( { not:
      { term: { viewed_subscriber_ids: req.userSession.user_id }
    }});
  }
...
ObservationsController.popularFieldValues = function( req, callback ) {
  ...
  countQuery.aggs = {
    terms: {
      ...
      aggs: { popular_term_values: { terms: {
        field: "annotations.concatenated_attr_val",
        size: req.query.per_page || 3
      } } }
    }
  };
  countQuery.per_page = 0;
  ...
  ObservationsController.elasticResults( countReq, function( err, data ) {
    ...
    var buckets = _.map( data.aggregations.terms.popular_term_values.buckets, function( b ) {
    ...
    // var termBucket = _.map( _.keys( controlledTermsIDs ), id => ( { controlled_term_id: id } ) );
    ESModel.fetchInstancesByIDsObject( controlledTermsIDs, ControlledTerm, { }, function( err, terms ) {
      _.each( terms, t => (
      ...
      if( err ) { return callback( err ); }
      _.each( buckets, b => {
        if( terms[ b.controlled_attribute_id ] ) {
          b.controlled_attribute = terms[ b.controlled_attribute_id ];
          delete b.controlled_attribute_id;
        }
        if( terms[ b.controlled_value_id ] ) {
          b.controlled_value = terms[ b.controlled_value_id ];
          delete b.controlled_value_id;
        }
      });
      buckets = _.filter( buckets, b => (
        b.controlled_attribute && b.controlled_value
      ));
      callback( null, {
        total_results: _.keys( buckets ).length,
        page: 1,
