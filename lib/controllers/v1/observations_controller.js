...
const LangUtils = require( "../../lang_utils" );
...
ObservationsController.create = async req => {
  ...
  await Observation.preloadInto( req, arr, localeOpts );
...
const postProcessTranslate = async (results, req) => {
  try {
    const { locale } = req.query;
    console.log("locale =", locale);

    const currentLocale = locale ? String(locale).split("-")[0] : "";
    if (!currentLocale || !results || !Array.isArray(results.results)) {
      return results;
    }

    results.results = await Promise.all(results.results.map(async obs => {
      // Comments
      if (Array.isArray(obs.comments)) {
        obs.comments = await Promise.all(obs.comments.map(async c => {
          const body = c?.body || "";
          if (!body.trim()) return c;

          const detected = await LangUtils.detect(body);
          const detectedBase = detected ? String(detected).split("-")[0] : "";

          console.log("comment detectedLanguage =", detected);

          const shouldTranslate =
            detectedBase && detectedBase !== currentLocale;

          if (!shouldTranslate) return c;

          const translatedBody = await LangUtils.translate(body, currentLocale, detected);
          console.log("comment translatedBody(sample) =", translatedBody?.slice?.(0, 60));

          return {
            ...c,
            body: translatedBody || body,
            original_body: body,
            original_language: detected || null
          };
        }));
      }

      // Identifications
      if (Array.isArray(obs.identifications)) {
        obs.identifications = await Promise.all(obs.identifications.map(async i => {
          const body = i?.body || "";
          if (!body.trim()) return i;

          const detected = await LangUtils.detect(body);
          const detectedBase = detected ? String(detected).split("-")[0] : "";

          console.log("ident detectedLanguage =", detected);

          const shouldTranslate =
            detectedBase && detectedBase !== currentLocale;

          if (!shouldTranslate) return i;

          const translatedBody = await LangUtils.translate(body, currentLocale, detected);
          console.log("ident translatedBody(sample) =", translatedBody?.slice?.(0, 60));
    ...
    console.error("postProcessTranslate top-level error:", err?.message || err);
    return results;
  }
};

ObservationsController.show = async ( req, res ) => {
  console.log( "ObservationsController.show..." );
  ...
  return ObservationsController.searchCacheWrapper( req, res, { postProcess: postProcessTranslate } );
};

ObservationsController.searchCacheWrapper = async (req, res, opts = {}) => {
  let results = await ObservationsController.methodCacheWrapper(
    req,
  ...
  if (typeof opts.postProcess === "function") {
    try {
      // allow async post-processing
      results = await opts.postProcess(results, req);
    } catch (e) {
      // don’t break the request if your post-process throws
      // optionally console.error(e);
    }
  }

  return InaturalistAPI.respond(req, res, results);
};
...
ObservationsController.search = async ( req, options = { } ) => {
  ...
  await preloadMethod( req, data.results, localeOpts );
  if ( req.userSession ) {
    // preload some data for the logged-in user that will be
    // used when creating Observation instances below
    await req.userSession.getCuratedProjectsIDs( );
    await req.userSession.getTrustingUserIDs( );
  }
  if ( !( req.query.only_id && req.query.only_id !== "false" ) ) {
...
ObservationsController.loadNewProjects = async ( obs, options = {} ) => (
  // newProjectsContaining
  // TODO: limit simultaneous promises
...
ObservationsController.elasticResults = async function observationElasticResults(
  ...
  const noLookupParms = _.isEmpty( _.without( _.keys( req.inat ),
    "preferredPlace",
    "isV2",
    "fields",
    "fieldRequested" ) );
...
ObservationsController.histogramCacheWrapper = async req => (
  ObservationsController.methodCacheWrapper( req,
    ObservationsController.histogram,
    "ObservationsController.histogram" )
);
...
ObservationsController.prepareElasticDataForResponse = ( data, req ) => {
...
ObservationsController.leafCounts = async req => (
  ESModel.ancestriesSpeciesCounts( req,
...
ObservationsController.umbrellasContainingObsTraditionalProjects = async obs => {
  ...
  return _.compact( _.uniq( _.flatten( _.map(
...
ObservationsController.newProjectsContaining = async ( obs, options = {} ) => {
  ...
  let matchingJoinedCollectionProjectIDs = [];
  if ( !_.isEmpty( joinedCollectionProjectIDs ) ) {
    ...
    const joinedCollectionProjects = _.compact( _.map( joinedCollectProjectDocs, "_source" ) );
    ...
    matchingJoinedCollectionProjectIDs = _.map( matchingJoinedCollectionProjects, "id" );
  }
  ...
  if (
    ...
    const curatedProjectsIDs = await options.userSession.getCuratedProjectsIDs( );
    if ( options.userSession.user_id === obs.user.id || !_.isEmpty( curatedProjectsIDs ) ) {
      const [
        observationTrustedProjectIDs,
...
ObservationsController.observationTrustedProjects = async (
  obs, obsSearchParams, options = { }
) => {
  let potentialTrustedProjectIDs = [];
  let observationTrustedProjectIDs = [];
  let trustedUmbrellasContainingCollectionsTrustedImplicitly = [];
  const curatedProjectsIDs = await options.userSession.getCuratedProjectsIDs( );
  ...
  if ( trustedProjectIDs.length > 0 ) {
    ...
    observationTrustedProjectIDs = _.intersection(
      trustedProjectIDs,
    ...
    trustedUmbrellasContainingCollectionsTrustedImplicitly = _.intersection(
      trustedProjectIDs,
...
ObservationsController.treeTaxa = async req => {
  ...
  const r = await TaxaController.speciesCountsResponse( req,
    leafCounts,
    { perPageOverride: leafCounts.length } );
...
ObservationsController.identifiers = async req => {
  ...
  if ( !req.query.no_total_hits ) {
    countQuery.aggs.total = {
      cardinality: {
        field: "non_owner_identifier_user_ids",
...
ObservationsController.observers = async ( req, options = { } ) => {
  const { perPage, offset, aggSize } = InaturalistAPI.paginationData( req, {
    default: 500, max: 500
  } );
  ...
  let firstCountMethod;
  let secondCountMethod;
  if ( req.query.order_by === "species_count" && aggSize > 0 ) {
    firstCountMethod = ObservationsController.observationsSpeciesObserverCounts;
    secondCountMethod = ObservationsController.observationsObserverCounts;
  } else {
    firstCountMethod = ObservationsController.observationsObserverCounts;
    secondCountMethod = ObservationsController.observationsSpeciesObserverCounts;
  }

  const firstRoundObservers = await firstCountMethod( _.cloneDeep( req ), options );
  if ( _.isEmpty( firstRoundObservers.counts ) ) {
    return ObservationsController.observationsObserversResponse(
      req, [firstRoundObservers], options
    );
  }

  const secondRoundQuery = _.assignIn( { }, req.query );
  if ( options.overrideAggSize && req.query.order_by !== "species_count" ) {
    // when setting an overrideAggSize and sorting by observation_count,
    // the request will contain its requested page and perPage values. These
    // can be used to filter by just the userIDs for the page that will be returned
    secondRoundQuery.user_id = _.map( _.orderBy(
      firstRoundObservers.counts, ["observation_count", "user_id"], ["desc", "asc"]
    ).slice( offset, offset + perPage ), "user_id" );
  } else {
    secondRoundQuery.user_id = _.keys( firstRoundObservers.counts );
  }
  const secondRoundRequest = _.assignIn( { }, req, { query: secondRoundQuery } );
  const secondRoundObservers = await secondCountMethod( secondRoundRequest, {
    isSecondRoundQuery: true
  } );
  return ObservationsController.observationsObserversResponse(
    req, [firstRoundObservers, secondRoundObservers], options
  );
};

ObservationsController.observationsObserversResponse = async (
  req, observers, options = { }
) => {
  const userIndexedCounts = _.merge( {}, ..._.map( observers, "counts" ) );
  ...
  if ( !options.skipLoadUsers ) {
    await ESModel.fetchBelongsTo( userCounts, User );
  }
  _.each( userCounts, uc => {
    if ( !uc.user ) {
      uc.user = { id: uc.user_id };
    }
  } );
  return {
    total_results: _.find( observers, "total" )?.total || 0,
    page,
    per_page: perPage,
    results: userCounts
  };
...
ObservationsController.observationsObserverCounts = async ( req, options = { } ) => {
  const countQuery = _.assignIn( { }, req.query );
  ...
  let { perPage, offset, aggSize } = InaturalistAPI.paginationData( req,
    { default: 500, max: 500 } );
  if ( options.overrideAggSize ) {
    aggSize = options.overrideAggSize;
    perPage = aggSize;
    offset = 0;
  }

  countQuery.aggs = { };
  if ( options.isSecondRoundQuery ) {
    aggSize = _.min( [aggSize, countQuery.user_id.length] );
    perPage = aggSize;
    offset = 0;
  }
  if ( !req.query.no_total_hits ) {
  ...
  return {
    total: data.aggregations.total_observers
      ? data.aggregations.total_observers.value : _.size( counts ),
    counts
  };
};

ObservationsController.observationsSpeciesObserverCounts = async ( req, options = { } ) => {
  const countQuery = _.assignIn( { }, req.query );
  let { perPage, offset, aggSize } = InaturalistAPI.paginationData( req,
    { default: 500, max: 500 } );
  if ( options.overrideAggSize ) {
  ...
  countQuery.aggs = { };
  if ( options.isSecondRoundQuery ) {
    perPage = _.min( [aggSize, countQuery.user_id.length] );
    aggSize = perPage;
    offset = 0;
  }
...
ObservationsController.updates = async req => {
  ...
  const response = await esClient.search( "update_actions", {
    body: {
  ...
  const localeOpts = util.localeOpts( req );
  await Comment.preloadInto( req, hits, localeOpts );
  await Identification.preloadInto( req, hits, localeOpts );
  return {
