...
ObservationsController.similarSpecies = ( req, callback ) => {
  ...
  Taxon.findByID( req.query.taxon_id, ( err, taxon ) => {
    ...
    const countQuery = _.assignIn( { }, req.query, {
      ...
      without_taxon_id: taxon.id,
      hrank: taxon.rank
    } );
    countQuery.aggs = {
      categories: { terms: { field: "ident_taxon_ids", size: 1000, exclude: taxon.ancestor_ids } }
    };
    countQuery.per_page = 0;
    const countReq = _.assignIn( { }, req, { query: countQuery } );
    ObservationsController.elasticResults( countReq, ( errr, data ) => {
      ...
      ESModel.fetchBelongsTo( buckets, Taxon, taxonOpts, errrr => {
        ...
        TaxaController.assignAncestors( { }, _.map( buckets, "taxon" ), { localeOpts, ancestors: true }, err4 => {
          ...
          _.each( buckets, b => {
            const bucketAncestors = b.taxon.ancestors.concat( [b.taxon] ).reverse( );
            let bucketSameRankTaxon;
            _.each( bucketAncestors, a => {
              if ( a.rank_level === taxon.rank_level && a.id !== taxon.id ) {
                bucketSameRankTaxon = a;
              }
            } );
          ...
          callback( null, {
            total_results: sameRankResults.length,
            page: 1,
            per_page: sameRankResults.length,
            results: _.reverse( _.sortBy( _.values( sameRankResults ), "count" ) )
          } );
        } );
      } );
    } );
