...
ObservationsController.searchCacheWrapper = async req => (
  new Promise( ( resolve, reject ) => {
    ...
    CachemanCache.cachedOrFetchFile( fileCacheKey, ( ) => {
      ObservationsController.search( req ).then( response => {
        CachemanCache.cachedCallbackFile( fileCacheKey, 60 * 60, ( err, val ) => {
...
ObservationsController.search = async req => {
  ...
  if ( req.query.include_new_projects ) {
    await ObservationsController.loadNewProjectsPromise( data.results );
  }
  await preloadCallback( data.results, localeOpts );
  if ( !( req.query.only_id && req.query.only_id !== "false" ) ) {
...
ObservationsController.loadNewProjectsPromise = async obs => (
  // newProjectsContaining
...
ObservationsController.elasticResults = async function observationElasticResults( req ) {
  ...
  return ESModel.elasticResultsAsync( req, query, "observations", opts );
};
  ...
  new Promise( ( resolve, reject ) => {
    ...
    CachemanCache.cachedOrFetchFile( fileCacheKey, ( ) => {
      ObservationsController.speciesCounts( req,
        CachemanCache.cachedCallbackFile( fileCacheKey, 60 * 60 * 2, ( err, data ) => {
          if ( err ) { return void reject( err ); }
          resolve( data );
        } ) );
    }, ( err, data ) => {
...
ObservationsController.taxaCountsByMonth = ( req, callback ) => {
  ...
  const countReq = _.assignIn( { }, req, { query: countQuery } );
  ObservationsController.elasticResults( countReq ).then( data => {
...
ObservationsController.qualityGrades = ( req, callback ) => {
  ...
  ObservationsController.elasticResults( req ).then( data => {
    ...
    ObservationsController.observationsSpeciesObserverCounts( _.cloneDeep( req ),
      ( err, speciesObservers ) => {
        ...
        ObservationsController.observationsObserverCounts( spReq, ( errr, observers ) => {
          if ( errr ) { return void callback( errr ); }
          ObservationsController.observationsObserversResponse( req, observers, speciesObservers ).then( response => {
...
ObservationsController.observationsObserversResponse = async (
  ...
  await ESModel.fetchBelongsToAsync( userCounts, User );
  return {
  ...
  Taxon.findByID( req.query.taxon_id ).then( taxon => {
    ...
    ObservationsController.elasticResults( countReq ).then( data => {
      ...
      ESModel.fetchBelongsTo( buckets, Taxon, taxonOpts, errrr => {
        if ( errrr ) { return void callback( errrr ); }
        TaxaController.assignAncestors( { }, _.map( buckets, "taxon" ), { localeOpts, ancestors: true } ).then( ( ) => {
