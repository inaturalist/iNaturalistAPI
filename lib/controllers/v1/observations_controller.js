...
ObservationsController.projectRulesQueryFilters = function( collection, callback ) {
  ...
  ObservationsController.reqToElasticQueryComponents( { query: collectParamsHash }, ( err, params ) => {
    if ( err ) { return callback( err ); }
    callback( null, params.search_filters );
  });
};
...
ObservationsController.projectsQueryFilters = function( projects, callback ) {
  ...
  const requests = projects.reduce( ( promiseChain, project ) => {
    return promiseChain.then( ( ) => new Promise( resolve => {
      ObservationsController.projectRulesQueryFilters( project, ( err, filters ) => {
        if ( err ) { return callback( err ); }
        queryFilters.push( { project, filters } );
        resolve( );
      });
    }));
  }, Promise.resolve( ) );
...
ObservationsController.applyCollectionProjectRules = function( req, callback ) {
  const projects = req.inat.project;
  delete req.inat.project;
  const queryProjectIDs = util.paramArray( req.query.project_id );
  ...
  _.each( projects, project => {
    if ( project.project_type === "umbrella" ) {
      ...
      _.pull( queryProjectIDs, project.id );
  ...
  ObservationsController.projectsQueryFilters( projectsWithRules, ( err, queryFilters ) => {
    ...
    const shoulds = [];
    newQuery.filters = newQuery.filters || [];
    // all the project filter must be true
    _.each( queryFilters, q => {
      shoulds.push( { bool: { must: q.filters } } );
    });
    // add this projects' filters to a big OR query
    newQuery.filters.push( { bool: { should: shoulds } } );
    // reset project_id to exlude the IDs of these collection projects
    newQuery.project_id = queryProjectIDs;
    // now process the initial query using this huge OR query as a filter
    ObservationsController.reqToElasticQueryComponents( { query: newQuery }, callback );
  });
}
...
ObservationsController.reqToElasticQueryComponents = function( req, callback ) {
  ...
  if( params.project_id && !( _.isArray( params.project_id ) && _.isEmpty( params.project_id ))) {
    ...
    // TODO: should add the collection project filters as an OR with project_ids
    if( params.pcid ) {
      if( params.pcid == "true" ) {
        search_filters.push( esClient.termFilter(
...
module.exports = {
  ...
  identificationCategories: ObservationsController.identificationCategories,
  ...
  umbrellaProjectStats: ObservationsController.umbrellaProjectStats,
  popularFieldValues: ObservationsController.popularFieldValues,
  qualityMetrics: ObservationsController.qualityMetrics,
  qualityGrades: ObservationsController.qualityGrades,
  ...
  treeTaxa: ObservationsController.treeTaxa,
