...
ObservationsController.identifiers = function( req, callback ) {
  ...
  if ( offset + perPage <= 500 ) {
    countQuery.aggs.nested.aggs.filtered.aggs.users = {
      terms: { field: "identifications.user.id", size: offset + perPage }
    };
  }
...
ObservationsController.observationsObserverCounts = function( req, callback ) {
  var countQuery = _.extend( { }, req.query );
  var size = _.isArray( countQuery.user_id ) ? countQuery.user_id.length : 500;
  const { page, perPage, offset } = InaturalistAPI.paginationData( req );
  size = size || ( offset + perPage ) || 500;
  size = size > 500 ? 500 : size;
  countQuery.aggs = {
    total_observers: { cardinality: { field: "user.id", precision_threshold: 10000 } }
  };
  if ( offset + perPage <= size ) {
    countQuery.aggs.top_observers = { terms: { field: "user.id", size: size } };
  }
  countQuery.per_page = 0;
  var countReq = _.extend( { }, req, { query: countQuery });
  ObservationsController.elasticResults( countReq, function( err, data ) {
    if( err ) { return callback( err ); }
    var counts = [];
    if ( page * perPage + offset <= size ) {
      counts = _.object( _.map( data.aggregations.top_observers.buckets, function( b ) {
