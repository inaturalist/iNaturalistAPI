...
ObservationsController.popularFieldValues = async req => {
  ...
  if ( req.query.taxon_id ) {
    const missingQuery = _.assignIn( { size: 0 }, req.query );
    missingQuery.aggs = {};
    ...
    _.each( controlledAttributeIds, controlledAttributeID => {
      missingQuery.aggs[`missing_${controlledAttributeID}`] = {
        filter: {
          bool: {
            must_not: [
              {
    ...
    const missingReq = _.assignIn( { }, req, { query: missingQuery } );
    missingData = await ObservationsController.elasticResults( missingReq );
  }
  const payload = {
    total_results: results.length,
    page: 1,
  ...
  if ( missingData ) {
    payload.without_annotations = _.reduce( missingData.aggregations, ( memo, agg, key ) => {
