...
ObservationsController.search = async req => {
  ...
  if (
    ...
    if ( nonTradQueryProjects.length > 0 ) {
...
ObservationsController.newProjectsContaining = async ( obs, options = {} ) => {
  ...
  const data = await ESModel.elasticResults( { query: { } }, query, "projects" );
  const collectionProjectIDs = _.map( data.hits.hits, "_source.id" );
  let joinedCollectionProjectIDs = _.map(
  ...
  if ( !_.isEmpty( collectionProjectIDs ) ) {
    ...
    joinedCollectionProjectIDs = _.uniq(
      joinedCollectionProjectIDs.concat( umbrellaProjectIDs )
    );
  }
  // Perform an additional query for projects joined and trusted by the observer
  // and curated by the viewer
  const trustedProjectsQuery = Object.assign( {}, query, {
    size: 100,
    _source: ["id"]
  } );
  // if the obs is obscured && the viewer curates collection projects
  if (
    ...
    if ( obs.geoprivacy === "private" || obs.geoprivacy === "obscured" ) {
      ...
      const projectsTrustedByObserver = await ProjectUser.projectsTrustedByUserFor( obs.user.id, "any" );
      // adjust the query to target those project IDs
      potentialTrustedProjectIDs = potentialTrustedProjectIDs.concat( projectsTrustedByObserver );
    // else (obs obscured by taxon_geoprivacy)
    } else {
      // load ids of all projects the observer trusts for when obscured by taxon
      const projectsTrustedByObserverForTaxon = await ProjectUser.projectsTrustedByUserFor( obs.user.id, "taxon" );
      // adjust the query to target those project IDs
      potentialTrustedProjectIDs = potentialTrustedProjectIDs.concat(
        projectsTrustedByObserverForTaxon
      );
    }
    // adjust query with project IDs the viewer curates AND are trusted by the observer
    const trustedProjectIDs = _.intersection(
    ...
    if ( trustedProjectIDs.length > 0 ) {
      ...
      trustedProjectsQuery.filters = _.map( trustedProjectsQuery.filters, f => {
        if (
          f.bool
          && f.bool.should
        ) {
          return Object.assign( {}, f, {
            bool: Object.assign( {}, f.bool, {
              should: _.map( f.bool.should, s => {
                if ( s.terms && s.terms["search_parameter_fields.place_id"] ) {
                  return esClient.termFilter( "search_parameter_fields.place_id", obs.private_place_ids );
                }
                return s;
              } )
            } )
      ...
      trustedProjectsQuery.filters.push( {
        terms: { id: trustedProjectIDs }
      } );
      // fetch additional projects using the modified query
      const trustedProjectsResult = await ESModel.elasticResults(
        { query: { } },
        trustedProjectsQuery,
        "projects"
      );
      joinedCollectionProjectIDs = _.uniq(
        joinedCollectionProjectIDs.concat(
          _.map( trustedProjectsResult.hits.hits, "_source.id" )
        )
      );
    }
  }
  return joinedCollectionProjectIDs;
};
