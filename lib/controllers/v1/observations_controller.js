...
ObservationsController.search = ( req, callback ) => {
  ...
  ObservationsController.resultsForRequest( req, ( err, data ) => {
    ...
    const preloadAndReturn = results => {
      preloadCallback( results, localeOpts, errr => {
...
ObservationsController.elasticResults = function observationElasticResults( req, callback ) {
  ObservationsController.reqToElasticQuery( req, ( err, query ) => {
    if ( err ) { return void callback( err ); }
    const opts = { excludes: ["taxon.names", "taxon.photos", "taxon.taxon_photos"] };
...
ObservationsController.reqToElasticQueryComponents = ( req, callback ) => {
  ...
  if ( params.popular === "true" ) {
    searchFilters.push( { range: { cached_votes_total: { gte: 1 } } } );
  } else if ( params.popular === "false" ) {
    searchFilters.push( esClient.termFilter( "cached_votes_total", 0 ) );
  }
  ...
  if ( params.geoprivacy === "open" ) {
    inverseFilters.push( { exists: { field: "geoprivacy" } } );
  } else if ( params.geoprivacy === "obscured_private" ) {
    searchFilters.push( esClient.termFilter( "geoprivacy", ["obscured", "private"] ) );
  } else if ( params.geoprivacy && params.geoprivacy !== "any" ) {
    const geoprivacyTermFilter = esClient.termFilter( "geoprivacy", params.geoprivacy );
    if ( geoprivacyTermFilter.terms.geoprivacy.indexOf( "open" ) < 0 ) {
      searchFilters.push( geoprivacyTermFilter );
    } else {
      ...
      geoprivacyTermFilter.terms.geoprivacy = _.filter(
        geoprivacyTermFilter.terms.geoprivacy, g => g !== "open"
      );
      searchFilters.push( {
        bool: {
          should: [
  ...
  if ( params.changed_since ) {
    const changedDate = moment.utc( params.changed_since ).parseZone( );
    if ( changedDate && changedDate.isValid( ) ) {
      ...
      if ( params.changed_fields ) {
        ...
        nestedQuery.nested.query.bool.filter.push( {
          terms: { "field_change_times.field_name": util.paramArray( params.changed_fields ) }
        } );
      }
      if ( params.change_project_id ) {
        ...
        nestedQuery.nested.query.bool.filter.push( {
          or: [
            {
              terms: {
                "field_change_times.project_id": util.paramArray( params.change_project_id )
              }
            },
...
ObservationsController.prepareElasticDataForReponse = ( data, req, callback ) => {
  const obs = _.map( data.hits.hits, h => (
    req.query.only_id ? h._source : new Observation( h._source, { userSession: req.userSession } )
  ) );
...
ObservationsController.identificationCategories = ( req, callback ) => {
  ...
  ObservationsController.elasticResults( countReq, ( err, data ) => {
    if ( err ) { return void callback( err ); }
    const buckets = _.map( data.aggregations.nested.categories.buckets, b => (
