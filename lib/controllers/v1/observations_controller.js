...
ObservationsController.reqToElasticQueryComponents = ( req, callback ) => {
  ...
  if ( params.q ) {
    ...
    switch ( searchOn ) {
      case "names":
        fields = ["taxon.names.name", "taxon.names_*"];
        break;
      case "tags":
        fields = ["tags"];
        break;
  ...
  if ( params.appropriate === "false" ) {
    searchFilters.push( { term: { spam: true } } );
    searchFilters.push( { term: { "user.spam": true } } );
    searchFilters.push( { term: { "user.suspended": true } } );
  } else if ( !params.appropriate || params.appropriate === "true" ) {
    searchFilters.push( { term: { spam: false } } );
    searchFilters.push( { term: { "user.spam": false } } );
    searchFilters.push( { term: { "user.suspended": false } } );
  }
