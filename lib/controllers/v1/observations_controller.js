...
ObservationsController.reqToElasticQueryComponents = function( req, callback ) {
  ...
  if ( params.ident_user_id ) {
    ...
    let nestedFilter;
    if ( Number( vals[0] ) ) {
      nestedFilter = esClient.termFilter( "identifications.user.id", params.ident_user_id );
    } else {
      nestedFilter = esClient.termFilter( "identifications.user.login", params.ident_user_id );
    }
    search_filters.push( {
      bool: {
        should: [
          esClient.termFilter( "identifier_user_ids", params.ident_user_id ),
          {
            nested: {
