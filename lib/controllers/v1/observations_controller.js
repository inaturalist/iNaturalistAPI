...
ObservationsController.histogram = function( req, callback ) {
  ...
  countQuery.observed_after = moment( Date.parse( "1800" ) ).format( );
  countQuery.aggs = {
    top_months: { terms: { field: "observed_on_details.month", size: 0 } },
    top_years: { terms: { field: "observed_on_details.year", size: 0 } },
    top_weeks: { terms: { field: "observed_on_details.week", size: 0 } }
  };
  ...
  ObservationsController.elasticResults( countReq, "observations", function( err, data ) {
    ...
    callback( null, {
      ...
      per_page: 3,
      results: {
        weeks: util.aggBucketsToHash( data.aggregations.top_weeks.buckets, { backfill: { min: 1, max: 53 } } ),
        months: util.aggBucketsToHash( data.aggregations.top_months.buckets, { backfill: { min: 1, max: 12 } } ),
        years: util.aggBucketsToHash( data.aggregations.top_years.buckets, { backfill: { max: 2016 } } )
      }
    });
  });
};
