...
const postProcessTranslate = ( results, req ) => {
  ...
  if ( currentLocale ) {
    ...
    if ( results && Array.isArray( results.results ) ) {
      results.results = results.results.map( obs => {
        // Comments
        if ( Array.isArray( obs.comments ) ) {
          obs.comments = obs.comments.map( c => {
            const detectedLanguage = LangUtils.detect( c.body );
            console.log( "detectedLanguage = ", detectedLanguage );
            if ( detectedLanguage !== currentLocale ) {
              const translatedBody = LangUtils.translate( c.body, currentLocale );
              console.log( "translatedBody = ", translatedBody );
              return {
                ...c,
                body: translatedBody,
        ...
        if ( Array.isArray( obs.identifications ) ) {
          obs.identifications = obs.identifications.map( i => {
            const detectedLanguage = LangUtils.detect( i.body );
            console.log( "detectedLanguage = ", detectedLanguage );
            if ( detectedLanguage !== currentLocale ) {
              const translatedBody = LangUtils.translate( i.body, currentLocale );
              console.log( "translatedBody = ", translatedBody );
              return {
                ...i,
                body: translatedBody,
