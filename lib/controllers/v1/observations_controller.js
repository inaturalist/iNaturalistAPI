...
ObservationsController.resultsForRequest = function( req, callback ) {
  ObservationsController.elasticResults( req, function( err, data ) {
    ...
    function handleObs( obs ) {
      obs = _.map( obs, function( o ) {
        return new Observation( o, { userSession: req.userSession } );
      });
      if( err ) { return callback( err ); }
    ...
    // If this is an authenticated request or all details request, preload
    // project observation preferences so we can show/hide coordinates correctly
    var obs = data.hits.hits.map( h => h._source );
    if ( req.userSession || req.query.details === "all" ) {
      Observation.preloadProjectObservationPreferences( obs, { userSession: req.userSession }, function( err ) {
        if( err ) { return callback( err ); }
        handleObs( obs );
      } )
    } else {
