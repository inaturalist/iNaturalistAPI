...
TaxaController.searchQuery = async ( req, opts = { } ) => {
  ...
  const taxa = _.map( data.hits.hits, h => {
    ...
    t.prepareForV1Response( localeOpts, mainTaxonOptions );
    return t;
  } );
  const wikiOpts = { ...options, ...localeOpts };
  await Taxon.preloadIntoTaxonPhotos( taxa, { localeOpts } );
...
TaxaController.speciesCountsResponse = async ( req, leafCounts, opts = { } ) => {
  ...
  if ( ( req.query.include_ancestors && req.query.include_ancestors !== "false" )
    || ( req.inat && req.inat.taxonAncestries )
  ) {
...
TaxaController.getChecklistSuggestions = async req => {
  ...
  const listResult = await pgClient.replica.query( listQuery.toString( ) );
  let query = squel
    .select( )
    .fields( [
  ...
  const listedTaxaResult = await pgClient.replica.query( query.toString( ) );
  ...
  if ( req.suggestParams ) {
    ...
    if ( req.suggestParams.taxon_id ) {
      ...
      if ( response.queryTaxon ) {
        ...
        response.queryTaxon.prepareForV1Response( localeOpts );
...
TaxaController.fetchSeekExceptionList = async ( ) => {
  ...
  const lookupMethod = async ( ) => {
    ...
    const listedTaxaResults = await pgClient.replica.query( listQuery.toString( ) );
...
TaxaController.mapLayers = async req => {
  ...
  let results = await pgClient.replica.query( query.toString( ) );
  ...
  query = squel.select( ).field( "1" )
    ...
    .limit( 1 );
  results = await pgClient.replica.query( query.toString( ) );
  const isInGeomodel = !_.isEmpty( results.rows );
