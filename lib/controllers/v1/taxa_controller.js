...
var TFServingTaxa = fs.readFileSync( __dirname + "/../../../public/taxa.txt" ).
  toString( ).split( "\n" ).map( i => ( Number( i ) ) );
...
TaxaController.searchQuery = function( req, options, callback ) {
  ...
  esClient.connection.search({
    ...
    Model.preloadTaxonPhotoDimensions( taxa, ( ) => {
      Taxon.preloadPhotosInto( taxa, { localeOpts: util.localeOpts( req ) }, ( ) => {
...
TaxaController.assignAncestors = function( req, taxa, options, callback ) {
  if( !options.details && !options.ancestorDetails ) { return callback( taxa ); }
  _.each( taxa, t => {
...
TaxaController.speciesCountsResponse = function( req, leafCounts, callback ) {
  ...
  if ( req.inat && req.inat.taxon ) {
    filters.push( { term: { ancestor_ids: req.inat.taxon.id } } );
  }
...
TaxaController.tensorflowServingScores = function( req, callback ) {
  if( !TFServingStub ) {
    const protoDescriptor = grpc.load(
      __dirname + "/../../../proto/prediction_service.proto" );
    var tfserving = protoDescriptor.tensorflow.serving;
    TFServingStub = new tfserving.PredictionService(
      global.config.imageProcesing.tfservingURL,
  ...
  var uploadPath = path.resolve( global.config.imageProcesing.uploadsDir,
    req.file.filename );
  const buffers = [ fs.readFileSync( uploadPath ) ];
  const msg = {
    model_spec: {
      name: "inception",
  ...
  TFServingStub.predict( msg, ( err, result ) => {
    if ( err ) { return callback( err ); }
    var scores = [ ];
    for ( var i = 0; i < result.outputs.classes.int_val.length; i++ ) {
      var klass = result.outputs.classes.int_val[i]
      var score = result.outputs.scores.float_val[i]
      scores.push({ taxon_id: TFServingTaxa[klass], count: score });
    }
    callback( null, scores );
  } );
}

TaxaController.flaskAPIScores = function( req, callback ) {
  ...
  fetch( global.config.imageProcesing.url, { method: "POST", body: form, timeout: 5000 } ).
    then( res => ( res.json( ) ) ).
    then( json => {
      // util.unlinkIfExists( uploadPath );
...
TaxaController.similarToImage = function( req, callback ) {
  if( !( global.config.imageProcesing && global.config.imageProcesing.url &&
         global.config.imageProcesing.uploadsDir && global.config.imageProcesing.tfservingURL ) ) {
    return callback({ error: "Not Found", status: 404 });
  }
  if( !req.file ) {
    return callback({ error: "File not provided", status: 404 });
  }
  req.inat = req.inat || { };
  req.inat.similarToImage = true;
  var visionBackend = req.query.version === "tfserving" ?
    TaxaController.tensorflowServingScores : TaxaController.flaskAPIScores
  visionBackend( req, ( err, scores ) => {
    if( err ) { return callback( err ); }
    scores = _.sortBy( scores, "count" ).reverse( ).slice( 0, 1000 );
    scores = _.filter( scores, r => ( r.count > 0.0001 ) );
    req.query.per_page = 1000;
    TaxaController.speciesCountsResponse( req, scores, ( err, response ) => {
      if( err ) { return callback( err ); }
      var sumOfCOunts = _.reduce( response.results, ( sum, r ) => ( sum + r.count ), 0);
      _.each( response.results, r => {
        r.score = ( r.count / sumOfCOunts ) * 100;
        delete r.count;
      });
...
module.exports = {
  ...
  similarToImage: TaxaController.similarToImage,
