...
TaxaController.exact = function( req, callback ) {
  ...
  const highlight = { fields: { "names.exact_ci": { } } };
  if( util.is_ja( q ) ) {
    filter.push( {
      nested: {
        path: "names",
        ...
        query: {
          bool: {
            must: [
              ...
              {
                terms: {
                  "names.place_taxon_names.place_id": preferredPlace.ancestor_place_ids
                }
              }
            ]
          }
  ...
  if( is_active !== null ) {
    filter.push( esClient.termFilter( "is_active", is_active ) );
  }
  req.query.page = 1;
...
TaxaController.autocomplete = function( req, callback ) {
  ...
  TaxaController.exact( {
    ...
    req.query.page = 1;
    // Make sure we don't show things that don't match at all
    const filter = [
      {
        nested: {
          path: "names",
    ...
    const highlight = { fields: { "names.name_autocomplete": { } } };
    const should = [];
    if ( localeOpts.locale ) {
      ...
      // multi-token matches should score higher
      should.push( {
        constant_score: {
          query: {
            nested: {
              ...
              query: {
                bool: {
                  must: [
                    ...
                    {
                      terms: {
                        "names.place_taxon_names.place_id": localeOpts.preferredPlace.ancestor_place_ids
                      }
                    }
                  ]
                }
    ...
    TaxaController.searchQuery( req, { }, function( err, response ) {
      if( err ) { return callback( err ); }
...
TaxaController.replaceInactiveTaxaCounts = ( counts, options, callback ) => {
  ...
  esClient.connection.search({
      ...
      if ( h._source.is_active === false ) {
        if ( _.isEmpty( h._source.current_synonymous_taxon_ids ) ) {
          ...
          _.each( h._source.current_synonymous_taxon_ids, synTaxonID => {
            // the synonym inherits the count of its parent inactive taxon
    ...
    const updatedCountsArray = _.map( countsObject, ( count, taxon_id ) => (
      { taxon_id: Number(taxon_id), count }
    ) );
    callback( null, updatedCountsArray, newTaxonIDs );
  });
}

TaxaController.search = function( req, callback ) {
  const searchReq = _.cloneDeep( req );
  searchReq.query.per_page = InaturalistAPI.perPage( req, { default: 30, max: 200 } );
  ...
  TaxaController.searchQuery( searchReq, { filters }, callback )
}
...
TaxaController.searchQuery = function( req, options, callback ) {
  ...
  var defaultSource = _.clone( TaxaController.returnFields );
  ...
  esClient.connection.search({
    ...
    // console.log( "\n[DEBUG] req.query.q: ", req.query.q );
    var taxa = _.map( data.hits.hits, function( h ) {
      // console.log(`${h._source.name} (${Math.log(1 + h._source.observations_count)}): ${h._score}`);
...
TaxaController.assignAncestors = function( req, taxa, options, callback ) {
  ...
  TaxaController.searchQuery( newReq, ancestorOpts, function( err, r ) {
    if( err ) { return callback( err ); }
    var ancestorsByID = _.fromPairs( _.map( r.results, r => [ r.id, r ] ));
    _.each( taxa, t => {
...
TaxaController.speciesCountsResponse = function( req, leafCounts, opts, callback ) {
  ...
  if ( offset >= paginationOptions.max ) {
  ...
  var leafCountObject = _.fromPairs( _.map( leafCounts, function( c ) {
