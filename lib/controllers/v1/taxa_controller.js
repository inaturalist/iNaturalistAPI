...
TaxaController.suggestifyRequest = async req => {
  ...
  if ( req.suggestParams.taxon_id ) {
    // Ensure taxon is genus or above
    const ancestors = await Taxon.ancestorsWithin( [req.suggestParams.taxon_id] );
    const ancestor = _.find( ancestors, a => a.rank_level >= 20 );
    if ( ancestor ) {
      req.suggestParams.taxon_id = ancestor.id;
    }
  }
...
TaxaController.getChecklistSuggestions = async req => {
  ...
  if ( req.suggestParams.taxon_id ) {
    listQuery = listQuery
      ...
      .where( "taxon_ancestors.taxon_id = ?", req.suggestParams.taxon_id );
  } else {
  ...
  if ( req.suggestParams.taxon_id ) {
    query = query
      ...
      .where( "taxon_ancestors.ancestor_taxon_id = ?", req.suggestParams.taxon_id );
  }
