...
TaxaController.replaceInactiveTaxaScores = ( scores, options, callback ) => {
  ...
  esClient.connection.search({
    ...
    if( err ) { return callback( err ); }
    const replacementTaxonIDs = { };
    const inactiveUnreplaced = { };
    _.each( data.hits.hits, h => {
      if ( h._source.current_synonymous_taxon_id ) {
        replacementTaxonIDs[h._source.id] = h._source.current_synonymous_taxon_id;
      } else if ( h._source.is_active === false ) {
        inactiveUnreplaced[h._source.id] = true;
      }
    });
    // replace inactive taxa
    _.each( scores, s => {
      if ( replacementTaxonIDs[s.taxon_id] ) {
        s.taxon_id = replacementTaxonIDs[s.taxon_id];
      }
    });
    // remove unreplaced inactive taxa
    if ( options.removeInactive === true ) {
      scores = _.filter( scores, s => !inactiveUnreplaced[s.taxon_id] );
    }
...
module.exports = {
  ...
  replaceInactiveTaxaScores: TaxaController.replaceInactiveTaxaScores,
