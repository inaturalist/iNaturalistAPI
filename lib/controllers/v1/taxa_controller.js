...
TaxaController.autocomplete = function( req, callback ) {
  ...
  TaxaController.exact( {
    ...
    if ( localeOpts.preferredPlace ) {
      const placeIDs = localeOpts.preferredPlace.ancestor_place_ids || [localeOpts.preferredPlace.id];
      should.push( {
        constant_score: {
          query: {
            // Within the should, though, we want a higher score if the taxon has a
...
TaxaController.assignAncestors = function( req, taxa, options, callback ) {
  ...
  var ancestor_ids = _.filter( _.uniq( _.flattenDeep( _.map( taxa, "ancestor_ids" ) ) ), _.identity );
...
TaxaController.assignPlaces = function( taxa, callback ) {
  var places = _.uniq( _.compact( _.flattenDeep( _.map( taxa, function( t ) {
    var place_ids = [ ];
