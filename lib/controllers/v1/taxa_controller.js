...
TaxaController.autocomplete = ( req, callback ) => {
  ...
  TaxaController.exact( {
    ...
    const q = params.q || params.term;
    if ( !q || q.length < 1 ) {
      return void callback( null, InaturalistAPI.basicResponse( req ) );
    }
    let isActive = true;
    if ( req.query.is_active === "false" ) {
      isActive = false;
...
TaxaController.wanted = ( req, callback ) => {
  ...
  esClient.connection.search( {
    ...
    esClient.connection.search( {
      ...
      ObservationsController.leafCounts( speciesCountsReq ).then( leafCounts => {
  ...
  esClient.connection.search( {
    ...
    Model.preloadTaxonPhotoDimensions( taxa, ( ) => {
      Taxon.preloadPhotosInto( taxa, { localeOpts }, err1 => {
        ...
        TaxaController.assignAncestors( req, taxa, options ).then( withAncestors => {
            ...
            TaxaController.assignPlaces( withChildren, ( err4, taxaWithPlaces ) => {
              ...
              Taxon.assignConservationStatuses( taxaWithPlaces, options, ( err5, taxawithCS ) => {
                ...
                Taxon.assignListedTaxa( taxawithCS, options, ( err6, taxawithLT ) => {
                  ...
                  Taxon.assignWikipediaSummary( taxawithLT, wikiOpts ).then( ( ) => {
                    callback( null, {
                      total_results: data.hits.total.value,
                      page: Number( searchHash.page ),
                      per_page: Number( searchHash.per_page ),
...
TaxaController.assignAncestors = ( req, taxa, options ) => {
  return new Promise( ( resolve, reject ) => {
    if ( !options.details && !options.ancestors ) {
      return void resolve( taxa );
    }
    _.each( taxa, t => {
    ...
    if ( _.size( ancestorIDs ) === 0 ) { return void resolve( taxa ); }
    const ancestorOpts = {
...
TaxaController.suggestifyRequest = req => {
  ...
  return new Promise( ( resolve, reject ) => {
    ...
    if ( req.query.observation_id ) {
      ...
      ObservationsController.elasticResults( obsReq ).then( data => {
        ...
        Observation.preloadMinimal( obs, { localeOpts: util.localeOpts( req ) } ).then( ( ) => {
          if ( obs[0].taxon ) {
...
TaxaController.getObservationSuggestions = req => {
  ...
  return new Promise( ( resolve, reject ) => {
    ObservationsController.leafCounts( req ).then( leafCounts => {
      TaxaController.speciesCountsResponse( req, leafCounts, { photos: true } ).then( data => {
        resolve( {
          comprehensiveness: {},
...
TaxaController.suggest = ( req, callback ) => {
  ...
  sugReqPromise.then( sugReq => {
    ...
    Taxon.preloadPhotosInto( taxa, { localeOpts }, err => {
      ...
      Taxon.assignWikipediaSummary( taxa, wikiOpts ).then( ( ) => {
...
TaxaController.fetchSeekExceptionList = callback => {
  if ( !config.seekExceptionListID ) {
    return void callback( );
  }
  const cacheKey = "SEEK_EXCEPTED_TAXA";
  CachemanCache.memoryCache.get( cacheKey, ( err, value ) => {
    if ( err ) { return void callback( err ); }
    if ( value ) {
    ...
    pgClient.connection.query( listQuery.toString( ), ( errr, listedTaxaResults ) => {
      if ( errr ) { return void callback( errr ); }
      SEEK_EXCEPTED_TAXA = _.fromPairs( _.map( listedTaxaResults.rows, r => [r.taxon_id, true] ) );
      CachemanCache.memoryCache.set( cacheKey, SEEK_EXCEPTED_TAXA, 3600, ( ) => {
        callback( null, SEEK_EXCEPTED_TAXA );
      } );
    } );
  } );
};
...
TaxaController.nearby = ( req, callback ) => {
  if ( !req.query.lat || !req.query.lng ) {
    return void callback( { error: "Must provide `lat` and `lng`", status: 422 } );
  }
  ...
  Promise.all( promises ).then( keyResponses => {
    ...
    if ( req.inat.returnRawCounts ) {
      return void callback( null, { results: allCounts } );
    }
    ...
    ESModel.fetchBelongsTo( topCounts, Taxon, taxonOpts, errr => {
      if ( errr ) { return void callback( errr ); }
      callback( null, {
