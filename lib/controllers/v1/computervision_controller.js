...
const ComputervisionController = class ComputervisionController {
  ...
  static async scoreImagePath( uploadPath, req ) {
    ...
    if ( req.body?.annotations || req.query.annotations ) {
      formData.append( "annotations", "true" );
      const annotationTaxonID = req.body?.annotation_taxon_id || req.query.annotation_taxon_id;
      if ( annotationTaxonID ) {
        formData.append( "annotation_taxon_id", annotationTaxonID );
      }
    }
  ...
  static async delegatedScoresProcessing( req, visionApiResponse ) {
    ...
    if ( visionApiResponse.annotations ) {
      response.annotations = visionApiResponse.annotations;
    }
