...
const ComputervisionController = class ComputervisionController {
  ...
  static async scoreImagePath( uploadPath, req ) {
    ...
    if ( config.imageProcesing.geomodel || req.query.geomodel ) {
      options.formData.geomodel = "true";
      if ( !req.body.skip_frequencies && req.body.lat && req.body.lng ) {
        options.formData.lat = req.body.lat;
        options.formData.lng = req.body.lng;
      }
    }
    try {
  ...
  static async combinedModelScoresProcessing( req, scores ) {
    ...
    if ( req.query.threshold && Number( req.query.threshold ) ) {
      // by default the threshold is an absolute value
      let thresholdScore = Number( req.query.threshold );
      if ( ( config.imageProcesing.geomodel || req.query.geomodel )
        && req.query.threshold_type === "percentage"
      ) {
        // set the threshold to a percentage of the top combined_score
        thresholdScore = ( thresholdScore / 100 ) * _.max( _.map( scores, "combined_score" ) );
      }
  ...
  static async scoreImageResponse( req, commonAncestor, topScores, options = { } ) {
    ...
    if ( req.query.geothresholds && !_.isEmpty( TFServingThresholds ) ) {
      response.experimental = "Geomodel Scoring, Geomodel Nearby";
    }
