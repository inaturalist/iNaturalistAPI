...
const ComputervisionController = class ComputervisionController {
  ...
  static async scoreImagePath( uploadPath, req ) {
    ...
    const allResultTaxonIDs = _.map( scores, "taxon_id" ).concat( newTaxonIDs );
  ...
  static async combinedModelScoresProcessing( req, scores ) {
    ...
    if ( req.body.must_be_in_frequency === "true" || req.body.frequency_only_remove === "true" ) {
      scores = _.filter( scores, s => s.frequency_score > 0 );
    }
    if ( req.body.must_be_in_vision === "true" ) {
      scores = _.filter( scores, s => s.vision_score > 0 );
    }
    ...
    if ( nearbyTaxa && _.has( nearbyTaxa, "results" ) ) {
      ...
      if ( !_.isEmpty( redisClient )
        ...
        topScores = _.map( taxonScores, ( v, k ) => (
          {
            taxon_id: k,
            combined_score: req.body.frequency_only_remove === "true" ? ( visionScores[k] || 0 ) : v,
        ...
        topScores = _.map( taxonScores, ( v, k ) => {
          ...
          return {
            taxon_id: k,
            combined_score: req.body.frequency_only_remove === "true"
              ? ( visionScores[k] || 0 )
              : baseVisScore,
  ...
  static async scoreImageResponse( req, commonAncestor, topScores, options = { } ) {
    ...
    if ( req.query.geomodel ) {
      response.experimental = "Geo Scoring, Frequency Nearby";
    }
