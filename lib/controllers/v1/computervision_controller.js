...
var ComputervisionController = class ComputervisionController {
  ...
  static scoreObservation( req, callback ) {
    ...
    if ( !obsID ) {
      return callback( { custom_message: "ID missing", status: 422 } );
    }
    ...
    ObservationsController.search( searchReq, ( err, response ) => {
      ...
      if ( !response || _.isEmpty( response.results ) ) {
        return callback( { custom_message: "Unknown observation" } );
      }
      ...
      if ( !photoURL ) { return callback( { custom_message: "Observation has no scorable photos" } ); }
  ...
  static scoreImageURL( req, options, callback ) {
    ...
    if ( !photoURL ) {
      return callback( { custom_message: "No scorable photo", status: 422 } );
    }
    // download the JPG
  ...
  static scoreImage( req, callback ) {
    ...
    return ComputervisionController.scoreImageUpload( req.file.path, req, callback );
  }
  ...
  static scoreImageUpload( uploadPath, req, callback ) {
    ...
    ComputervisionController.scoreImagePath( uploadPath, req, ( err, scores ) => {
      ...
      // Override scores for local testing
      scores = [
        { taxon_id: 7173, count: 0.7, },
        { taxon_id: 4938, count: 0.2, },
        { taxon_id: 797, count: 0.1, }
      ];
      if ( req.body.taxon_id ) {
        if ( !TFServingTaxonDescendants[req.body.taxon_id] ) {
          return util.basicResponse( null, req, null, callback );
        }
      ...
      if ( err ) { return callback( err ); }
        ComputervisionController.nearbyTaxonFrequencies( req, scores, commonAncestor, ( err, nearbyTaxa ) => {
          if ( err ) { return callback( err ); }
  ...
  static scoreImageAfterFrequencies( req, scores, nearbyTaxa, commonAncestor, callback ) {
    if ( nearbyTaxa && !_.isEmpty( nearbyTaxa.results ) ) {
      ...
      var sumScores = _.reduce( ancestorNearbyTaxaResults, ( sum, r ) => ( sum + r.count ), 0 );
      let frequencyScores = { };
      let visionScores = { };
      let taxonScores = { };
