...
const ComputervisionController = class ComputervisionController {
  ...
  static async representativePhotosQdrant( results, resultsToLookup, embedding ) {
    const qdrantResponse = await qdrantClient.query( "production_taxon_photos", {
      query: embedding,
      filter: {
        must: [{
          key: "ancestor_ids",
          match: {
            any: _.map( resultsToLookup, result => result.taxon.id )
          }
        }]
      },
      with_payload: true,
    ...
    _.each( results, result => {
      if ( result && result.taxon && result.taxon.default_photo ) {
        const firstMatch = _.find( qdrantResponse.points, p => (
          p?.payload?.photo_id && _.includes( p.payload.ancestor_ids, result.taxon.id )
        ) );
        if ( !firstMatch ) {
          return;
        }
        representativeTaxonPhotos.push( {
          taxon: result.taxon,
          photo_id: firstMatch.payload.photo_id
        } );
      }
    } );
    return representativeTaxonPhotos;
  }

  static async representativePhotosElasticsearch( results, resultsToLookup, embedding ) {
  ...
  static async addRepresentativePhotos( results, embedding ) {
    ...
    const lookupMethod = qdrantClient.connected( )
      ? ComputervisionController.representativePhotosQdrant
      : ComputervisionController.representativePhotosElasticsearch;

    let representativeTaxonPhotos = [];
    try {
      representativeTaxonPhotos = await lookupMethod( results, resultsToLookup, embedding );
