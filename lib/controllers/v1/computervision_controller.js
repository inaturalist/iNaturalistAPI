...
var ComputervisionController = class ComputervisionController {
  ...
  static scoreImagePath( uploadPath, req, callback ) {
    ...
    request.post( options, ( err, httpResponse, body ) => {
      ...
      TaxaController.replaceInactiveTaxaScores( scores, { removeInactive: true }, ( err, updatedScores ) => {
        if ( err ) { return callback( err ); }
        if ( req.inat && req.inat.visionCacheKey ) {
          FileCache.cacheFile( uploadPath, JSON.stringify( updatedScores ) );
        }
        callback( null, updatedScores );
      });
    });
  }
