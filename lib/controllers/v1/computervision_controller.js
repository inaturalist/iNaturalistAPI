...
const ComputervisionController = class ComputervisionController {
  static scoreObservation( req ) {
    return new Promise( ( resolve, reject ) => {
      ...
      ObservationsController.search( searchReq ).then( response => {
        ...
        ComputervisionController.scoreImageURL( req, { observation }, ( err, r ) => {
  ...
  static scoreImagePath( uploadPath, req, callback ) {
    ...
    request.post( options, ( err, httpResponse, body ) => {
      ...
      TaxaController.replaceInactiveTaxaCounts( counts, { removeInactive: true } ).then( r => {
  ...
  static scoreImageResponse( req, commonAncestor, top10, callback ) {
    ...
    TaxaController.speciesCountsResponse( req, top10, { } ).then( response => {
      ...
      // like names
      Taxon.findByID( commonAncestor.taxon.id ).then( taxon => {
  ...
  static addTaxa( speciesCountsReq, scores, callback ) {
    if ( !speciesCountsReq.inat.visionCacheKey ) {
      TaxaController.speciesCountsResponse( speciesCountsReq, _.cloneDeep( scores ) )
        .then( response => {
    ...
    TaxaController.speciesCountsResponse( speciesCountsReq, scoresToLookup ).then( response => {
      _.each( response.results, r => {
  ...
  static nearbyTaxonFrequenciesRedis( req, scores, commonAncestor, callback ) {
    ...
    TaxaController.nearby( nearbyReq ).then( response => {
  ...
  static nearbyTaxonFrequencies( req, scores, commonAncestor, callback ) {
    ...
    ObservationsController.speciesCounts( { query } ).then( response => {
      callback( null, response );
    } ).catch( callback );
  }
