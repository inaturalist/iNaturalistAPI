...
const ComputervisionController = class ComputervisionController {
  ...
  static async scoreImagePath( uploadPath, req ) {
    ...
    if ( config.imageProcesing.geomodel || req.query.geomodel ) {
      ...
      if ( !req.body.skip_frequencies && req.body.lat && req.body.lng ) {
        ...
        if ( req.body.taxon_id && !( req.userSession.isAdmin && req.query.skip_iconic_filter ) ) {
          formData.append( "taxon_id", req.body.taxon_id );
        }
      }
      if ( req.userSession.isAdmin && req.query.aggregated ) {
        formData.append( "aggregated", "true" );
      }
      if ( req.userSession.isAdmin && req.query.score_without_geo ) {
        formData.append( "score_without_geo", "true" );
      }
    }
  ...
  static async combinedModelScoresProcessing( req, scores ) {
    ...
    if ( req.userSession.isAdmin
      && ( req.query.test_feature === "geo_elevation" || req.query.geo_nearby ) ) {
      _.each( scores, s => {
