...
var ProjectsController = class ProjectsController {
  ...
  static searchCriteria( req, options = { } ) {
    ...
    if ( _.isEmpty( sort ) ) {
      ...
      } else if ( ( req.query.featured === "true" || req.query.noteworthy === "true" ) && ( req.query.order_by === "featured" ) ) {
        sort.push( {
  ...
  static followers( req, callback ) {
    let { page, perPage } = InaturalistAPI.paginationData( req, { default: 10, max: 100 } );
    var ids = _.filter( req.params.id.split( "," ), _.identity );
    var numericIDs = _.filter( ids, id => Number( id ) );
    if ( _.isEmpty( numericIDs ) ) { numericIDs = [ -1 ] }
    var query = squel.select( ).field( "s.user_id, count(*) OVER() AS total_count" ).
      from( "projects p" ).
      join( "subscriptions s", null, "s.resource_type='Project' AND s.resource_id=p.id" ).
  ...
  static feature( req, callback ) {
    InaturalistAPI.iNatJSWrap( projects.feature, req  ).then( function( ) {
      callback( null, true );
    }).catch( callback );
  }

  static unfeature( req, callback ) {
    InaturalistAPI.iNatJSWrap( projects.unfeature, req  ).then( function( ) {
      callback( null, true );
    }).catch( callback );
  }
