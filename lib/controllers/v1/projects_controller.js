...
const ProjectsController = class ProjectsController {
  ...
  static async search( req ) {
    ...
    const response = await ESModel.elasticResultsAsync( req, searchCriteria, "projects" );
  ...
  static async create( req ) {
    const r = InaturalistAPI.iNatJSWrap( projects.create, req );
    const arr = [{ project: r }];
  ...
  static async update( req ) {
    const r = InaturalistAPI.iNatJSWrap( projects.update, req );
    const arr = [{ project: r }];
    await Project.preloadInto( arr );
    return arr[0].project;
  }
  ...
  static async followers( req ) {
    ...
    const total = _.isEmpty( rows ) ? 0 : Number( rows[0].total_count );
    _.each( rows, r => ( delete r.total_count ) );
    await ESModel.fetchBelongsToAsync( rows, User, { source: { includes: ["id", "login", "icon"] } } );
    return {
      total_results: total,
      page,
      per_page: perPage,
  ...
  static async esResponseToAPIResponse( req, esResponse ) {
    ...
    await ESModel.fetchBelongsToAsync( withUsers, User );
