...
PlacesController.returnFields = [
  "id", "slug", "name", "display_name", "place_type", "admin_level",
  "bbox_area", "ancestor_place_ids", "geometry_geojson", "bounding_box_geojson",
  "location"];

PlacesController.autocomplete = ( req, callback ) => {
...
PlacesController.nearby = ( req, callback ) => {
  InaturalistAPI.setPerPage( req, { default: 10, max: 20 } );
  let placesQuery = _.assignIn( { }, req.query );
  // looking up standard places
  placesQuery.community = false;
  PlacesController.nearbyQuery( { query: placesQuery }, ( err, nearbyRsp ) => {
    ...
    PlacesController.nearbyQuery( { query: placesQuery }, ( errr, commRsp ) => {
...
PlacesController.nearbyQueryBody = req => {
  ...
  if ( req.query.community ) {
    ...
    filters.push( { range: { admin_level: { gte: -1 } } } );
  }
  if ( req.query.name ) {
...
PlacesController.nearbyQuery = ( req, callback ) => {
  const body = PlacesController.nearbyQueryBody( req );
  if ( _.isUndefined( body ) ) { return void callback( null, [] ); }
  esClient.connection.search( {
    preference: global.config.elasticsearch.preference,
    index: `${process.env.NODE_ENV || global.config.environment}_places`,
    body
...
PlacesController.show = ( req, callback ) => {
  const ids = _.filter( req.params.id.split( "," ), _.identity );
  if ( ids ) {
    req.query.per_page = ids.length;
  }
  let err = InaturalistAPI.setPerPage( req, { default: 20, max: 500 } );
  ...
  esClient.connection.search( {
    ...
    InaturalistAPI.basicResponse( errr, req, response, callback );
  } );
};

PlacesController.containing = ( req, callback ) => {
  const err = InaturalistAPI.setPerPage( req, { default: 20, max: 100 } );
  if ( err ) { return void callback( err ); }
  const floatLat = parseFloat( req.query.lat );
  const floatLng = parseFloat( req.query.lng );
  if ( ( !floatLat && floatLat !== 0 ) || ( !floatLng && floatLng !== 0 ) ) {
  ...
  if ( !req.query.include_community_places ) {
    filters.push( { terms: { admin_level: [0, 1, 2] } } );
  }
  esClient.connection.search( {
