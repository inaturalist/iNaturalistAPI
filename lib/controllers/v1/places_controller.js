...
PlacesController.nearbyQueryBody = function( req ) {
  ...
  var filters = [ { exists: { field: "geometry_geojson" } } ];
  var inverseFilters = [ ];
...
PlacesController.show = function( req, callback ) {
  var err = InaturalistAPI.setPerPage( req, { default: 20, max: 500 } );
  ...
  esClient.connection.search({
    ...
    body: {
      ...
      size: req.query.per_page,
      _source: { includes: PlacesController.returnFields }
    }
  }, function( err, response ) {
    InaturalistAPI.basicResponse( err, req, response, callback );
  });
...
PlacesController.containing = function( req, callback ) {
  var err = InaturalistAPI.setPerPage( req, { default: 20, max: 100 } );
  if( !err && !( parseFloat(req.query.lat) && parseFloat(req.query.lng ) ) ) {
    err = { error: "Must provide `lat` and `lng`", status: 422 };
  }
  if( err ) { return callback( err ); }
  var filters = [
