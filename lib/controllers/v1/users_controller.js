...
var UsersController = class UsersController {
  ...
  static nearbyUnobservedSpecies( req, callback ) {
    if( !req.params.user_id ) { return callback({ error: "Must include user_id", status: 422 }); }
    if( !req.query.lat ) { return callback({ error: "Must include lat", status: 422 }); }
    if( !req.query.lng ) { return callback({ error: "Must include lng", status: 422 }); }
    req.query.user_id = req.params.user_id;
    // return up to 10k previously observed species, not bounded by lat/lng
    var observedReq = { query: Object.assign( { }, req.query, {
      per_page: 10000, hrank: "species" } ) };
    delete observedReq.query.lat;
    delete observedReq.query.lng;
    ObservationsController.speciesCounts( observedReq, ( err, r ) => {
      var seenTaxonIDs = _.map( r.results, r => r.taxon.id );
      ...
      var unobservedReq = { query: Object.assign( { }, req.query, { per_page: 500,
        without_taxon_id: seenTaxonIDs, hrank: "species", lrank: "species" } ) };
