...
const UsersController = class UsersController {
  ...
  static async me( req ) {
    ...
    if ( user.taxon_name_priorities ) {
      const placeOptions = {
        source: {
          includes: ["id", "uuid", "name", "display_name"]
        }
      };
      await ESModel.fetchBelongsTo( user.taxon_name_priorities, Place, placeOptions );
    }
    return InaturalistAPI.resultsHash( {
      total: 1,
      per_page: 1,
  ...
  static async autocomplete( req ) {
    ...
    const filters = [];
    if ( !req.query.include_suspended ) {
      filters.push( esClient.termFilter( "suspended", false ) );
    }
    if ( req.query.project_id ) {
      ...
      const members = await project.allMembers( req.query );
      filters.push( esClient.termFilter( "id", _.isEmpty( members ) ? -1 : _.keys( members ) ) );
    }
  ...
  static async projects( req ) {
    InaturalistAPI.setRequestPaginationData( req, { default: 10, max: 100 } );
    const user = await User.findInES( req.params.id );
    if ( !user ) {
      throw httpError( 422, `User ${req.params.id} does not exist` );
    }
  ...
  static async preloadSubscriptionResources( req, response ) {
    ...
    await Promise.all( [
      ...
      ESModel.fetchBelongsTo(
        taxonSubscriptions,
        Taxon,
        { ...taxonOpts, ...sharedPreloadOptions }
      ),
  ...
  static async emailAvailable( req ) {
    if ( !req.applicationSession ) {
      throw httpError( 401, "Unauthorized" );
    }
    return InaturalistAPI.iNatJSWrap( users.emailAvailable, req );
  }
  ...
  static async posts( req ) {
    ...
    const query = squel.select( ).field( "posts.*, count(*) OVER() AS total_count" )
      .from( "posts" )
      .join( "users", null, "posts.parent_id = users.id" )
      .where( "parent_type = 'User'" )
      .where( "users.id IN ? OR users.login IN ?", numericIDs, ids )
