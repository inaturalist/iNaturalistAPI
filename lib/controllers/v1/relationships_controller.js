...
const RelationshipsController = class RelationshipsController {
  static async index( req ) {
    ...
    const orderBy = req.query.order_by === "friendships.id" ? "friendships.id" : "users.login";
    ...
    if ( req.query.user_id ) {
      query = query.where( "users.id = ?", req.query.user_id );
    }
    ...
    await Relationship.preloadInto( req, rows );
  ...
  static async create( req ) {
    ...
    const arr = [response];
    await Relationship.preloadInto( req, arr );
    return arr[0];
  }
