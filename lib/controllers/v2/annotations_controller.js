...
const destroy = async req => {
  ...
  await InaturalistAPI.iNatJSWrap( annotations.delete, req );
  return null;
};
...
const vote = async req => {
  ...
  await InaturalistAPI.iNatJSWrap( annotations.vote, req );
  return null;
};
...
const unvote = async req => {
  req.params.id = req.params.id || req.params.uuid;
  // If there are no errors, this will be a 204 and there's no response
  await InaturalistAPI.iNatJSWrap( annotations.unvote, req );
  return null;
};
