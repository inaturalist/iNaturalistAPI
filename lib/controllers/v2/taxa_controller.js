...
const suggest = async req => {
  if ( req.query.featured_observation_id ) {
    ...
    if ( observationsByUuid[req.query.featured_observation_id] ) {
      req.query.featured_observation_uuid = req.query.featured_observation_id;
...
const lifelistMetadata = async req => {
  const responseV1 = await ctrlv1.lifelistMetadata( req );
  return {
    page: 1,
    per_page: responseV1.results.length,
    total_results: responseV1.results.length,
    results: responseV1.results
  };
};
