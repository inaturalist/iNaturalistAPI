...
routes.observations_stats = function( req, res ) {
  InaturalistAPI.observationsStats( req, function( err, data ) {
    if( err ) { return util.renderError( err, res ); }
...
routes.species_count = function( req, res ) {
  InaturalistAPI.leafCount( req, function( err, data ) {
    if( err ) { return util.renderError( err, res ); }
    res.setHeader( "Cache-Control", "public, max-age=300" ); // 5 minutes
    res.jsonp( data );
  });
...
routes.observations_show = function( req, res ) {
  InaturalistAPI.fetchIDs( req, "observations", function( err, data ) {
    if( err ) { return util.renderError( err, res ); }
    res.jsonp( data );
  });
};
...
routes.taxa_show = function( req, res ) {
  InaturalistAPI.fetchIDs( req, "taxa", function( err, data ) {
    if( err ) { return util.renderError( err, res ); }
    res.jsonp( data );
  });
};
