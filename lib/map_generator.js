...
MapGenerator.createLayer = ( ) => {
  const layer = new mapnik.Layer( "tile", "EPSG:4326" );
  layer.styles = ["style"];
  return layer;
};
...
MapGenerator.convertGeometryGeoJsonFeature = ( geometry, value, colorScale ) => ( {
  type: "Feature",
  properties: {
    value,
...
MapGenerator.h3CountsToGeoJSON = ( json, colorScale ) => (
  _.map( json, ( value, h3Index ) => {
    const geoJSONCoords = h3.cellToBoundary( h3Index, true );
    let wrapsAntimeridian = false;
    const longitudes = _.map( geoJSONCoords, _.first );
    // any h3 polygon whose longitudes are very far apart are polygons that cross
    // the antimeridian with some vertices on one side, and other vertices on the other.
    // These need to be split into two polygons, one on each side of the antimeridian
    if ( _.max( longitudes ) > 90 && _.min( longitudes ) < -90 ) {
    ...
    if ( !wrapsAntimeridian ) {
      return MapGenerator.convertGeometryGeoJsonFeature( geometry, value, colorScale );
    }

    return MapGenerator.convertGeometryGeoJsonFeature(
      splitGeoJSON( geometry ), value, colorScale
    );
  } )
);
...
MapGenerator.pythonTilesGeojson = async (
  ...
  return MapGenerator.h3CountsToGeoJSON( json, colorScale );
};
