...
MapGenerator.postgisDatasource = req => {
  if ( POSTGIS_DATASOURCES[req.postgis.query] ) {
    return _.sample( POSTGIS_DATASOURCES[req.postgis.query] );
  }

  POSTGIS_DATASOURCES[req.postgis.query] = [];
  const datasourceConfig = {
    ...
    // default extent that will be overwritten at query time. This prevents
    // node-mapnik from querying the underlying query for max extent
    extent: [-180, -90, 180, 90]
  };
  if ( config.database.replicas ) {
    _.each( config.database.replicas, replica => {
      _.times( replica.weight || 1, ( ) => {
        POSTGIS_DATASOURCES[req.postgis.query].push( new mapnik.Datasource(
          { ...datasourceConfig, ...replica }
        ) );
      } );
    } );
  } else if ( config.database.replica ) {
    POSTGIS_DATASOURCES[req.postgis.query].push( new mapnik.Datasource(
      { ...datasourceConfig, ...config.database.replica }
    ) );
  } else {
    POSTGIS_DATASOURCES[req.postgis.query].push( new mapnik.Datasource(
      datasourceConfig
...
MapGenerator.finishMap = async ( req, res, map, layer, features ) => {
  ...
  if ( req.params.dataType === "postgis" ) {
    ...
    let zoom = parseInt( req.params.zoom, 10 );
    if ( req.largeTiles ) { zoom -= 1; }
    layer.datasource.extent = MapGenerator.merc.bbox(
      parseInt( req.params.x, 10 ),
      parseInt( req.params.y, 10 ),
      zoom
    );
