...
const VisionDataExporter = class VisionDataExporter {
  ...
  taxonHasMaximumSpatial( taxon ) {
    if ( _.size( this.taxonData[taxon.id].obsWithCoords ) < this.spatialMax ) {
      return false;
    }
    return true;
  }
  ...
  async distributeTaxonGeospatial( taxon, observations, options = { } ) {
    ...
    _.each( _.shuffle( observations ), observation => {
      ...
      if ( _.size( this.taxonData[taxon.id].obsWithCoords ) >= this.spatialMax ) { return; }
  ...
  async lookupObs( taxon ) {
    ...
    await this.forEachInSequence( [true, false], async community => {
    ...
    this.taxa[taxon.id].spatialCount = _.size( this.taxonData[taxon.id].obsWithCoords );
  ...
  async writeTaxonData( taxon ) {
    ...
    await this.writeSpatialToFile( taxon, taxonData.obsWithCoords );
  ...
  addObsToTaxonSpatialSet( observation, taxon, options = { } ) {
    ...
    this.taxonData[taxon.id].obsWithCoords.push( observation );
