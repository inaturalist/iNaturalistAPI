...
const VisionDataExporter = class VisionDataExporter {
  ...
  static MAX_POSITIONAL_ACCURACY = 50000;
  static OBSERVATION_QUERY_LIMIT = 200000;
  ...
  async export( ) {
    ...
    await this.processInnernodeSpatial( );
    ...
    await this.writeAllTaxa( );
    await this.closeOutputFiles( );
  }

  async processInnernodeSpatial( ) {
    this.leafTaxaNeedingProcessing = [];
    // nodes marked as "complete" are not leaf nodes themselves, rather the ancestors of "populated"
    // leaves. These are the "innernodes" of the taxonomy. Add them to the processing queue. Order
    // doesn't matter as they already know what spatial data to fetch, and they will only process
    // spatial data in this pass as they are "complete" nodes
    _.each( this.taxa, t => {
      if ( t.status === "complete" ) {
        this.leafTaxaNeedingProcessing.push( t );
      }
    } );
    await this.processTaxaStack( );
  }

  assignNestedSetValues( taxonID = 0, index = 0, depth = 0 ) {
    const childIDs = _.sortBy( _.keys( this.taxonChildren[taxonID] ) );
    _.each( childIDs, childID => {
      const child = this.taxa[childID];
      if ( !child ) {
        console.log( `Taxon ${childID} does not exist` );
        return;
      }
      child.left = index;
      child.depth = depth;
      index += 1;
      if ( !_.isEmpty( this.taxonChildren[childID] ) ) {
        index = this.assignNestedSetValues( childID, index, depth + 1 );
      }
      child.right = index;
      index += 1;
    } );
  ...
  assignInitialStatuses( taxonID = 0 ) {
    const childIDs = _.sortBy( _.keys( this.taxonChildren[taxonID] ) );
    _.each( childIDs, childID => {
      const child = this.taxa[childID];
      if ( !this.leafCandidate( child ) ) {
        child.status = "skipped";
      }
      if ( !this.isAncestorOrWithinFilterBranch( child ) ) {
        child.status = "skipped";
      }
      if ( !_.isEmpty( this.taxonChildren[childID] ) ) {
        this.assignInitialStatuses( childID );
      }
    } );
  }
  ...
  async prepareCandidateTaxonomy( ) {
    ...
    this.taxaPopulateAncestries( );
    this.assignNestedSetValues( );
    this.selectTaxa = _.compact( _.map( this.selectTaxonIDs, id => this.taxa[id] ) );
    this.assignInitialStatuses( );
    // prepare variaibles for assining classIDs
    this.taxonLeafClasses = { };
    this.taxonIconicClasses = { };
    this.taxonSpatialClasses = { };
    this.leafClassIndex = 0;
    this.iconicClassIndex = 0;
    this.spatialClassIndex = 0;
  }
  ...
  // be processed.
  depthFirstTraverse( taxon = null, spaces = 0 ) {
    if ( taxon ) {
    ...
    if ( !_.isEmpty( childIDs ) && !( taxon && _.includes( this.customLeafTaxonIDs, taxon.id ) ) ) {
      const children = _.map( _.sortBy( _.keys( childIDs ) ), childID => this.taxa[childID] );
      ...
      if ( !_.isEmpty( unprocessedChildren ) ) {
        _.each( unprocessedChildren, child => {
          this.depthFirstTraverse( child, spaces + 1 );
        } );
        return;
      }
  ...
  async taxaProcessor( options = { } ) {
    ...
    _.each( rows, row => {
      this.taxa[row.id] = row;
      this.taxa[row.id].iconic_taxon_id = this.taxa[row.id].iconic_taxon_id || 0;
    } );
  }

  taxaPopulateAncestries( ) {
    _.each( _.sortBy( this.taxa, ["is_active", "observations_count"] ).reverse( ), taxon => {
      const ancestorIDs = _.isEmpty( taxon.ancestry ) ? [] : taxon.ancestry.split( "/" );
      ancestorIDs.push( taxon.id );
      let lastAncestorID = 0;
      _.each( ancestorIDs, ancestorID => {
        const useAncestry = this.validateAncestry( ancestorID, lastAncestorID, taxon.id );
        // prevent duplicate pathways
        if ( useAncestry ) {
  ...
  taxonHasMinimumPhotos( taxon ) {
    if ( taxon.status === "complete" ) { return true; }
  ...
  leafCandidate( taxon ) {
    if ( this.extinctTaxonIDs[taxon.id] ) { return false; }
    if ( !taxon.is_active ) { return false; }
    if ( taxon.rank_level < 10 ) { return false; }
    if ( taxon.rank === "hybrid" ) { return false; }
    if ( taxon.rank === "genushybrid" ) { return false; }
    return true;
  }

  // return IDs of this taxon plus all its descendants
  lookupTaxonIDs( taxon ) {
    if ( !_.isEmpty( this.selectTaxonIDs ) && _.every( this.selectTaxonIDs, selectTaxonID => {
      const selectTaxon = this.taxa[selectTaxonID];
      const isExcludedByTaxonFilter = selectTaxon
        && !( taxon.left >= selectTaxon.left && taxon.right <= selectTaxon.right );
      return isExcludedByTaxonFilter;
    } ) ) {
      return [];
    }
    let ids = [];
    if ( taxon.is_active && taxon.observations_count > 0 ) {
      ids.push( taxon.id );
    }
    const childIDs = _.keys( this.taxonChildren[taxon.id] );
    _.each( childIDs, childID => {
      const child = this.taxa[childID];
      if ( child.status === "complete" || child.status === "populated" ) { return; }
      ids = ids.concat( this.lookupTaxonIDs( child ) );
    } );
    return ids;
  }
  ...
  async validObservationsOfTaxon( taxon, options = { } ) {
    const taxonIDs = this.lookupTaxonIDs( taxon );
    ...
    let query = squel.select( )
      ...
      .limit( VisionDataExporter.OBSERVATION_QUERY_LIMIT );
  ...
  async distributeTaxonGeospatial( taxon, observations, options = { } ) {
    ...
    _.each( _.shuffle( observations ), observation => {
      ...
      // observations must not be voted as having an inaccurate location
      if ( observation.inaccurateLocation ) { return; }
      // coordinates much be within a certain accuracy
      if ( observation.positional_accuracy > this.maxPositionalAccuracy ) { return; }
      if ( observation.geoprivacy === "private" ) { return; }
  ...
  async writeTaxonData( taxon ) {
    ...
    this.taxonSpatialClasses[taxon.id] = this.spatialClassIndex;
    this.spatialClassIndex += 1;
    ...
    this.taxa[taxon.id].status = this.taxa[taxon.id].status || "populated";
  }
  ...
  async writePhotosToFile( taxon, observationPhotos, file ) {
    await util.forEachInSequence( observationPhotos, async observationPhoto => {
      ...
      const photoFileFields = [
        photo.id,
        photo.medium_url,
        observationPhoto.observation.id,
  ...
  async writeSpatialToFile( taxon, observations ) {
    await util.forEachInSequence( observations, async observation => {
      const spatialFileFields = [
        ...
        this.taxonLeafClasses[taxon.id],
        this.taxonIconicClasses[taxon.iconic_taxon_id],
        this.taxonSpatialClasses[taxon.id],
        taxon.id,
    ...
    _.each( scores, ( metrics, observationID ) => {
      _.each( metrics, ( score, metric ) => {
        if ( score < 0 ) {
          ...
          if ( metric === "location" ) {
            observations[observationID].inaccurateLocation = true;
          }
        }
      } );
    } );
  ...
  async hiddenPhotos( photos ) {
    const ids = _.map( _.keys( photos ), k => Number( k ) );
    if ( _.isEmpty( ids ) ) { return; }
    const query = squel.select( )
      .field( "resource_id" )
      .field( "action" )
    ...
    _.each( sortedRows, row => {
      ...
      // is set then the photos hidden state has already been captured
      if ( _.has( photos[row.resource_id], "hidden" ) ) {
        return;
      }
      photos[row.resource_id].hidden = ( row.action === "hide" );
    } );
  }
  ...
  async observationPhotos( taxon, observations ) {
    ...
    await this.hiddenPhotos( observationPhotos );
    return _.omitBy( _.omitBy( observationPhotos, "flagged" ), "hidden" );
  }
  ...
  isAncestorOrWithinFilterBranch( taxon ) {
    if ( _.isEmpty( this.selectTaxa ) ) { return true; }
    return !_.every( this.selectTaxa, selectTaxon => {
      const isOutsideSelectBranch = (
        ( selectTaxon.left < taxon.left && selectTaxon.right < taxon.right )
        || ( selectTaxon.left > taxon.left && selectTaxon.right > taxon.right ) );
      return isOutsideSelectBranch;
    } );
  }
  ...
  async createCSVFiles( ) {
    ...
    await this.writeToFile( this.taxonomyFileStream,
      "parent_taxon_id,taxon_id,rank_level,leaf_class_id,iconic_class_id,spatial_class_id,name,iconic_taxon_id,rank" );

    this.allTaxaFileStream = fs.createWriteStream(
      path.join( this.outputDir, "all_taxa.csv" ), { flags: "w" }
    );
    await this.writeToFile( this.allTaxaFileStream,
      "parent_taxon_id,taxon_id,leaf_class_id,iconic_class_id,spatial_class_id,rank,rank_level,iconic_taxon_id,left,right,observations_count,is_active,name,status,train,val,test,spatial" );
    ...
    await this.writeToFile( this.spatialFileStream,
      "observation_id,latitude,longitude,observed_on,leaf_class_id,iconic_class_id,spatial_class_id,taxon_id,obs_taxon_id,community,captive,positional_accuracy" );
    ...
    await util.forEachInSequence( ["train", "val", "test"], async set => {
      ...
      await this.writeToFile( this[streamName], "photo_id,photo_url,observation_id,leaf_class_id,iconic_class_id,taxon_id,obs_taxon_id,community" );
    } );
  }
  ...
  //
  // lookup all file extensions and populate an instance variable with the results
  //
  ...
  async writeAllTaxa( taxonID = 0 ) {
    const childIDs = _.sortBy( _.keys( this.taxonChildren[taxonID] ) );
    await util.forEachInSequence( childIDs, async childID => {
      const taxon = this.taxa[childID];
      const taxaFileFields = [
        ...
        _.has( this.taxonSpatialClasses, taxon.id ) ? this.taxonSpatialClasses[taxon.id] : "",
        taxon.rank,
        taxon.rank_level,
      ...
      await this.writeToFile( this.allTaxaFileStream, taxaFileFields.join( "," ) );
      if ( !_.isEmpty( this.taxonChildren[childID] ) ) {
        await this.writeAllTaxa( childID );
      }
    } );
  }
  ...
  async printTree( taxon = null, ancestorLinePrefix = "", linePrefix = "" ) {
    if ( taxon ) {
      const taxaFileFields = [
        ...
        _.has( this.taxonSpatialClasses, taxon.id ) ? this.taxonSpatialClasses[taxon.id] : "",
        taxon.name ? taxon.name.replace( /,/g, "" ) : "",
        taxon.iconic_taxon_id === 0 ? "" : taxon.iconic_taxon_id,
        taxon.rank
      ];
      ...
      if ( taxon.id !== Taxon.life.id ) {
        const counts = `, ${taxon.trainCount}::${taxon.valCount}::${taxon.testCount}::${taxon.spatialCount}`;
  ...
  async printStats( ) {
    ...
    const spatialTotal = _.sum( _.map( _.keys( this.taxonSpatialClasses ),
