...
function photoJob( imageData ) {
  return new Promise( resolve => {
    if ( argv.download ) {
      CVParser.cachePhoto( imageData ).then( resolve );
      return;
    }
    CVParser.cachePhoto( imageData ).then( cachePath => (
      CVParser.cachedScoreImage( cachePath, imageData, runOptions( ) )
    ) ).then( scores => {
      let matchingIndex;
...
function startParsing( ) {
  ...
  parser.readTestData( argv.path ).then( ( ) => (
