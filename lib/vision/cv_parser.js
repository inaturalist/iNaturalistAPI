...
const CVParser = class CVParser {
  ...
  static async fetchScoresForImage( imagePath, imageData, options = { } ) {
    const body = { };
    const query = { };
    body.observation_id = imageData.observation_id;
    ...
    if ( options.geothresholds ) {
      query.geothresholds = true;
    }
    _.each( scoreImageOptions, option => {
      if ( options[option] ) {
        // make the body property a string, since express params will be strings
        body[option] = options[option].toString( );
      }
    } );
    const visionOptions = {
      visionStats: true
    };
    if ( options.vision_cache_key ) {
      visionOptions.visionCacheKey = `${imagePath}-${options.vision_cache_key}`;
    }
  ...
  parallelProcessPhotos( processor, concurrency = 1, maxIterations = 0 ) {
    return new Promise( ( resolve, reject ) => {
      if ( processor.constructor.name !== "AsyncFunction" ) {
        reject( new Error( "CVParser :: processor must be a Function" ) );
        return;
      }
      ...
      const promiseProducer = ( ) => {
        ...
        if ( processDataIndex % 100 === 0 ) {
          ...
          const secondsLeft = ( _.min( [_.size( this.testData ), maxIterations] )
            - processDataIndex ) / perSecond;
