...
const {
  host, hosts, apiKey, tlsAllowUnauthorized
...
esClient.buildIndexName = index => {
  ...
  if ( process.env.INAT_ES_INDEX_PREFIX ) {
    return `${process.env.INAT_ES_INDEX_PREFIX}_${index}`;
  }
  // Use env by default
  return `${process.env.NODE_ENV}_${index}`;
};
...
esClient.envelopeFilter = filter => {
  ...
  if ( !( opts.nelat
    ...
    || opts.swlat
    || opts.swlng
    || _.isNumber( opts.nelat )
    || _.isNumber( opts.nelng )
    || _.isNumber( opts.swlat )
    || _.isNumber( opts.swlng )
  ) ) {
    return null;
  }
  opts.swlng = parseFloat( _.isNumber( opts.swlng ) ? opts.swlng : ( opts.swlng || -180 ) );
  opts.swlat = parseFloat( _.isNumber( opts.swlat ) ? opts.swlat : ( opts.swlat || -90 ) );
  opts.nelng = parseFloat( _.isNumber( opts.nelng ) ? opts.nelng : ( opts.nelng || 180 ) );
  opts.nelat = parseFloat( _.isNumber( opts.nelat ) ? opts.nelat : ( opts.nelat || 90 ) );
  ...
  if ( opts.nelat > 90 ) { opts.nelat = 90; }
  if ( _.isNumber( opts.nelng ) && _.isNumber( opts.swlng ) && opts.nelng < opts.swlng ) {
    // the envelope crosses the antimeridian. Unfortunately, elasticsearch
    // doesn't handle this well and we need to split the envelope at
    // the antimeridian and do an OR query
...
esClient.deleteIndex = async ( options = { } ) => {
  ...
  const response = await esClient.connection.indices
    .exists( { index: esClient.buildIndexName( options.name ) } );
  if ( response === true ) {
    await esClient.connection.indices.delete(
      { index: esClient.buildIndexName( options.name ) }
    );
  }
};
