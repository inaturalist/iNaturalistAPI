...
const util = class util {
  ...
  static observationSearchRequestCacheKey( req, prefix, options = { } ) {
    ...
    if ( prefix === "ESModel.ancestriesSpeciesCounts"
      || queryDup.page === "1"
      || queryDup.page === 1
    ) {
    ...
    if ( prefix === "ObservationsController.identifiers"
      ...
      delete reqInatDup.taxonNamePrioritiesHash;
    }
    ...
    cacheableParams.include_ancestors = queryDup.include_ancestors
      ? queryDup.include_ancestors : null;
    cacheableParams.fields = queryDup.fields
      ? crypto
        .createHash( "md5" )
        .update( JSON.stringify( queryDup.fields ), "utf8" )
        .digest( "hex" ) : null;
    cacheableParams.taxonNamePrioritiesHash = reqInatDup.taxonNamePrioritiesHash;
    if ( prefix === "ObservationsController.identifiers"
      || prefix === "ObservationsController.speciesCounts"
      || prefix === "ObservationsController.observers"
    ...
    delete queryDup.return_bounds;
    delete queryDup.locale;
    delete queryDup.no_total_hits;
    delete queryDup.preferred_place_id;
    delete queryDup.photos;
    delete queryDup.include_ancestors;
    delete queryDup.fields;
    delete queryDup.queryKey;
    delete queryDup.pageParam;
    delete queryDup.direction;
    delete queryDup.meta;
    delete queryDup.signal;
    delete queryDup.client;
    delete reqInatDup.preferredPlace;
    delete reqInatDup.place;
    delete reqInatDup.taxonNamePrioritiesHash;
    delete reqInatDup.isV2;
    delete reqInatDup.fieldRequested;
    delete reqInatDup.fields;
    if ( _.isEmpty( queryDup.taxon_id ) && !_.isNumber( queryDup.taxon_id ) ) {
      delete queryDup.taxon_id;
    }
    if ( queryDup.order_by === "" ) {
      delete queryDup.order_by;
    }
    // if there are no lat/lng parameters, radius has no effect on results and
    // can be ignored when considering what to cache
    if ( _.isEmpty( queryDup.lat )
      && _.isEmpty( queryDup.lng )
      && !_.isNumber( queryDup.lat )
      && !_.isNumber( queryDup.lng ) ) {
      delete queryDup.lat;
      delete queryDup.lng;
