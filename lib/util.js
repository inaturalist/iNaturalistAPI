...
const util = class util {
  ...
  static accessControlHeaders( req, res, next ) {
    ...
    res.header( "Access-Control-Allow-Headers",
      "Origin, X-Requested-With, Content-Type, Accept, Authorization, Access-Control-Allow-Methods, X-Via" );
  ...
  static observationSearchRequestCacheKey( req, prefix, options = { } ) {
    ...
    const placeID = queryDup.place_id ? queryDup.place_id : null;
    ...
    if ( _.isEmpty( queryDup ) && _.isEmpty( reqInatDup ) ) {
      ...
      if ( perPage && !options.ignorePagination ) {
        fileCacheKey += `-pp-${perPage}`;
      }
