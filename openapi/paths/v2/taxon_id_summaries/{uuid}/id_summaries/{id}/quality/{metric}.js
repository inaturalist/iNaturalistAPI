...
module.exports = sendWrapper => {
  async function POST( req, res ) {
    await IdSummaryQualityMetricsController.setSummaryQualityMetric( req );
    sendWrapper( req, res.status( 204 ) );
  }

  POST.apiDoc = {
    tags: ["TaxonIdSummaries"],
    summary: "Vote on an ID summary quality metric",
    "x-unpublished": true,
    security: [{
      userJwtRequired: []
    }],
  ...
  async function DELETE( req, res ) {
    await IdSummaryQualityMetricsController.deleteSummaryQualityMetric( req );
    sendWrapper( req, res.status( 204 ) );
  }

  DELETE.apiDoc = {
    tags: ["TaxonIdSummaries"],
    summary: "Remove the authenticated user's vote on an ID summary quality metric",
    "x-unpublished": true,
    security: [{
      userJwtRequired: []
    }],
